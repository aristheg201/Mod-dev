...
final class PlatformIntegrations implements AutoCloseable {
    ...
    private PlatformIntegrations(MinecraftServer server, CobblemonPartyBridge cobblemon, PlaceholderBridge placeholders,
                                 Registry<RewardProvider> rewardProviders, Set<String> capabilities, Map<String,String> unavailable) {
    ...
    static PlatformIntegrations discover(MinecraftServer server) {
        ...
        Registry.Builder<RewardProvider> rewards = new Registry.Builder<>(); CobblemonPartyBridge cobblemon = null; PlaceholderBridge placeholders = null;
        ...
        if (IntegrationDetector.installed("economy")) unavailable.put("economy", "provider lacks nonblocking exactly-once transaction API");
        for (String capability : List.of("svquest", "svframe")) if (IntegrationDetector.installed(capability)) unavailable.put(capability, "adapter contract unavailable");
        return new PlatformIntegrations(server, cobblemon, placeholders, rewards.build(), capabilities, unavailable);
    }
