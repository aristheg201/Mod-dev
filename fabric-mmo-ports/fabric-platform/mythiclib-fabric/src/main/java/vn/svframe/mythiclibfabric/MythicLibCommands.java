...
final class MythicLibCommands {
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> statTree() {
        ...
        stat.then(literal("add")
                .then(argument("player", EntityArgumentType.player())
                        .then(argument("stat", StringArgumentType.word()).then(value))));
        return stat;
    }
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> cooldownTree() {
        ...
        cooldown.then(literal("reduce")
                ...
                .then(literal("initial")
                        .then(argument("key", StringArgumentType.string())
                                .then(argument("player", EntityArgumentType.player())
                                        .then(argument("percent", DoubleArgumentType.doubleArg(0.0d, 1.0d))
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> tempStatTree() {
        ...
        temp.then(literal("add")
                .then(argument("player", EntityArgumentType.player())
                        .then(argument("stat", StringArgumentType.word())
                                .then(argument("value", StringArgumentType.string())
                                        .then(argument("duration", LongArgumentType.longArg(1L))
                                                .executes(ctx -> statAdd(ctx.getSource(), EntityArgumentType.getPlayer(ctx, "player"), StringArgumentType.getString(ctx, "stat"), StringArgumentType.getString(ctx, "value"), LongArgumentType.getLong(ctx, "duration"), UUID.randomUUID().toString(), false)))))));
        temp.then(literal("remove")
                .then(argument("player", EntityArgumentType.player())
                        .then(argument("stat", StringArgumentType.word())
                                .then(argument("key", StringArgumentType.string())
                                        .executes(ctx -> statRemove(ctx.getSource(), EntityArgumentType.getPlayer(ctx, "player"), StringArgumentType.getString(ctx, "stat"), StringArgumentType.getString(ctx, "key")))))));
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> debugTree() {
        ...
        debug.then(literal("cast")
                .then(argument("skill", StringArgumentType.word())
                        .executes(ctx -> cast(ctx.getSource(), StringArgumentType.getString(ctx, "skill"), null))));
        debug.then(literal("stats")
                .then(argument("player", EntityArgumentType.player())
                        .executes(ctx -> dumpStats(ctx.getSource(), EntityArgumentType.getPlayer(ctx, "player")))));
        ...
        debug.then(literal("versions").executes(ctx -> {
            success(ctx.getSource(), "MythicLib 1.7.1 behavior port | Minecraft 1.21.1 | Fabric");
            return 1;
        }));
        debug.then(literal("parse")
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> healthScale(String name) {
        return literal(name).requires(source -> permitted(source, HEALTH_SCALE))
                .then(argument("scale", DoubleArgumentType.doubleArg(0.0001d))
                        .executes(ctx -> healthScaleSet(ctx.getSource(), ctx.getSource().getPlayerOrThrow(), DoubleArgumentType.getDouble(ctx, "scale"))))
                .then(argument("player", EntityArgumentType.player())
                        .then(argument("scale", DoubleArgumentType.doubleArg(0.0001d))
                                .executes(ctx -> healthScaleSet(ctx.getSource(), EntityArgumentType.getPlayer(ctx, "player"), DoubleArgumentType.getDouble(ctx, "scale")))));
    }
    ...
    private static int reload(ServerCommandSource source) {
        ...
        if (!ok) {
            source.sendError(Text.literal("MythicLib reload failed. Check server log."));
            return 0;
        }
    ...
    private static int damage(ServerCommandSource source, ServerPlayerEntity attacker, Entity target, double amount) {
        if (!(target instanceof LivingEntity living)) {
            source.sendError(Text.literal("Target is not a living entity."));
            return 0;
        }
    ...
    private static int statAdd(ServerCommandSource source, ServerPlayerEntity player, String stat, String encoded,
                               long duration, String key, boolean unique) {
        ParsedModifier parsed;
        try {
            parsed = parseModifier(encoded);
        } catch (IllegalArgumentException ex) {
            source.sendError(Text.literal("Invalid stat modifier value: " + encoded));
            return 0;
        }
        ...
        NativeStatEngine.Modifier modifier = new NativeStatEngine.Modifier(
                id, actualKey, parsed.value(), parsed.type(),
                NativeStatEngine.EquipmentSlot.OTHER, NativeStatEngine.ModifierSource.OTHER, expires);
        MythicLibStatMod.engine().register(player.getUuid(), stat, modifier);
    ...
    private static int cooldownCheck(ServerCommandSource source, String key, ServerPlayerEntity player) {
        NativePlayerData data = requireData(source, player);
    ...
    private static int cooldownReset(ServerCommandSource source, String key, ServerPlayerEntity player) {
        NativePlayerData data = requireData(source, player);
        if (data == null) return 0;
        data.cooldowns().reset(key);
        success(source, "Reset cooldown '" + key + "' for " + player.getGameProfile().getName());
        return 1;
    }

    /** Exact original command unit: duration argument is ticks, CooldownMap accepts seconds. */
    private static int cooldownApply(ServerCommandSource source, String key, ServerPlayerEntity player, long durationTicks) {
        NativePlayerData data = requireData(source, player);
        if (data == null) return 0;
        data.cooldowns().apply(key, durationTicks / 20.0d);
        success(source, "Applied " + durationTicks + " ticks of cooldown '" + key + "' to " + player.getGameProfile().getName());
        return 1;
    }
    ...
    private static int healthScaleSet(ServerCommandSource source, ServerPlayerEntity player, double scale) {
        MythicLibHealthScale.setScale(player, scale);
        success(source, "Health scale for " + player.getGameProfile().getName() + " set to " + scale);
        return 1;
    }

    private static int healthScaleReset(ServerCommandSource source, ServerPlayerEntity player) {
        MythicLibHealthScale.resetScale(player);
        success(source, "Health scale for " + player.getGameProfile().getName() + " reset to config/default behavior.");
        return 1;
    }

    private static int dumpStats(ServerCommandSource source, ServerPlayerEntity player) {
        if (MythicLibStatMod.engine().instances(player.getUuid()).isEmpty()) {
            success(source, player.getGameProfile().getName() + " has no materialized stat instances.");
            return 1;
        }
        for (NativeStatEngine.StatInstance instance : MythicLibStatMod.engine().instances(player.getUuid()))
            success(source, instance.stat() + "=" + instance.formatFinal() + " modifiers=" + instance.modifiers().size());
        return 1;
    }
    ...
    private static boolean permitted(ServerCommandSource source, String permission) {
        ...
        if (entity instanceof ServerPlayerEntity player) return MythicLibPermissionBridge.has(player, permission);
