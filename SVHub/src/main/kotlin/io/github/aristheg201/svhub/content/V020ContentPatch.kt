...
object V020ContentPatch {
    private const val MARKER="svhub_v020"
    fun apply(content:HubContent):HubContent?{
        if(content.pages.any{it.id==MARKER})return null
        if(content.pages.none{it.id=="svhub_v011"})return null
        ...
        val pages=content.pages.filterNot{it.id in setOf("gacha","skin_showcase","companions","native_platform","native_arcade","native_wallet",MARKER)}.map{if(it.id==home.id)patchedHome else it}+listOf(
            modulePage("native_platform","native","SV Platform","Native Gacha + Arcade, SkiesSkins-backed cosmetics.","dashboard","One client/server UI. Skin state is never duplicated in SVHub."),
    ...
    private fun modulePage(id:String,route:String,title:String,subtitle:String,module:String,body:String)=HubPage(id=id,route=route,category="native",title=LocalizedText.of(title),subtitle=LocalizedText.of(subtitle),theme="sv_native",tags=listOf("native",module,"svhub"),components=listOf(component("${id}_hero","animated_text","text" to "<bold>$title</bold>","animation" to "shimmer","align" to "center","scale" to 1.3),component("${id}_body","notice","text" to body),nativeButton("${id}_open","MỞ $title",module,"Mở native client UI; server giữ state.")))
