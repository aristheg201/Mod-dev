...
object NativeSkinService {
    ...
    internal fun grantBonusPokemon(player:ServerPlayer,perfectIvs:Int){if(perfectIvs<=0)return;val pool=PokemonSpecies.implemented.asSequence().filter{it.nationalPokedexNumber>0&&it.resourceIdentifier.namespace=="cobblemon"}.toList();val species=pool.randomOrNull()?:return;val cmd="givepokemonother ${player.gameProfile.name} ${species.resourceIdentifier} min_perfect_ivs=$perfectIvs";val ok=runCatching{player.server.commands.performPrefixedCommand(player.server.createCommandSourceStack(),cmd)}.getOrDefault(0)>0;if(!ok)player.sendSystemMessage(Component.literal("Skin đã grant qua SkiesSkins, nhưng Pokémon thưởng ${perfectIvs}IV chưa phát được; báo admin."))}
}
