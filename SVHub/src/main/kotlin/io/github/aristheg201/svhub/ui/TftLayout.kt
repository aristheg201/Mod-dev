...
object TftLayoutResolver {
    fun resolve(area: UiRect, density: UiDensity): TftResolvedLayout {
        ...
        val hudH = when (density) { UiDensity.COMPACT -> 22; UiDensity.REGULAR -> 30; UiDensity.WIDE -> 34 }
            .coerceAtMost(area.height / 3)
        ...
        val footer = UiRect(area.x + gap, (area.bottom - footerH).coerceAtLeast(contentTop + 24),
            (area.width - gap * 2).coerceAtLeast(40), footerH)
        ...
        val players = if (density == UiDensity.WIDE && sideH >= 94) {
            ...
            val playerH = (sideH * 58 / 100).coerceAtLeast(42)
                .coerceAtMost((sideH - reserveForItems).coerceAtLeast(42))
