...
object SVHub : ModInitializer {
    ...
    override fun onInitialize() {
        ...
        SVHubApi.registerIntegration(HubIntegrationDescriptor(
            "svhub:native", "SV Native Platform",
            setOf("wallet", "gacha", "skins", "matchmaking", "chess", "xiangqi", "ludo", "uno", "cards", "tft", "tower_defense")
        ))
        ...
        NativePlatformNetwork.registerCommon()
        SVHubCommands.register()
        NativeCommands.register()
        ServerTickEvents.END_SERVER_TICK.register(VanillaCompanionService::tick)
        ServerTickEvents.END_SERVER_TICK.register(NativePlatform::tick)
        ...
        ServerLifecycleEvents.SERVER_STARTED.register { server ->
            ...
            SVHubRuntime.store.initializeAsync().whenComplete { snapshot, error ->
                server.execute {
                    ...
                    var candidate = snapshot.content
                    var changed = false
                    V011ContentPatch.apply(candidate)?.let { candidate = it; changed = true }
                    V020ContentPatch.apply(candidate)?.let { candidate = it; changed = true }
                    if (!changed) {
                    ...
                    // Both bundled migrations publish atomically as one new revision.
                    SVHubRuntime.store.commitAsync(snapshot.revision, candidate).whenComplete { result, patchError ->
                        server.execute {
                            if (patchError != null) LOGGER.error("SVHub native content upgrade failed", patchError)
                            else if (!result.ok) LOGGER.error("SVHub native content upgrade rejected: {}", result.message)
                            else LOGGER.info("SVHub native platform installed at revision {}", result.revision)
                            SVHubNetwork.broadcastHello()
        ...
        ServerPlayConnectionEvents.JOIN.register { handler, _, _ ->
            ...
            NativePlatform.onJoin(handler.player)
        }
        ServerPlayConnectionEvents.DISCONNECT.register { handler, _ ->
            NativePlatformNetwork.close(handler.player.uuid)
            NativePlatform.onDisconnect(handler.player)
