...
object NativeRanking {
    ...
    fun start(path: java.nio.file.Path) {
        if (!java.nio.file.Files.exists(path)) io.github.aristheg201.svhub.util.AtomicFiles.writeUtf8(path, javaClass.getResourceAsStream("/data/svhub/ranking.json")!!.bufferedReader().use { it.readText() })
