...
import vn.svframe.svrelationships.fabric.command.*;
...
public final class SVRelationshipsFabric implements ModInitializer {
    ...
    @Override public void onInitialize(){
        ...
        var scheduleDefinitions=new ScheduleDefinitionService(root);scheduleDefinitions.initialize();
        var dialogueDefinitions=new DialogueDefinitionService(root);dialogueDefinitions.initialize();
        var guiDefinitions=new GuiDefinitionService(root);var guiReload=guiDefinitions.reload();if(!guiReload.success())throw new IllegalStateException("Unable to load GUI definitions: "+guiReload.detail());
        var messages=new MessageService(config);
        var householdRepository=new HouseholdRepository(root.resolve("state/households.json"));householdRepository.load();var households=new HouseholdService(householdRepository,config);
