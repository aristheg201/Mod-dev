...
public final class AnniversaryService {
    ...
    public AnniversaryService(AnniversaryDefinitionService definitions, RelationshipService relationships, RelationshipRewardService rewards) {
        this.definitions=definitions; this.relationships=relationships; this.rewards=rewards;
    }
    ...
    public Result claim(ServerPlayerEntity player, UUID pokemonId, String anniversaryId, long nowMillis) {
        ...
        String flag="anniversary."+anniversaryId+".cycle";
        long lastCycle=-1;
        String stored=state.flag(flag);
        if(stored!=null){try{lastCycle=Long.parseLong(stored);}catch(NumberFormatException ignored){lastCycle=-1;}}
        ...
        if(!definition.rewardProfile().isBlank()){
            var rewardResult=rewards.claim(player,pokemonId,definition.rewardProfile(),nowMillis);
            if(rewardResult==RelationshipRewardService.ClaimResult.DELIVERY_FAILED||rewardResult==RelationshipRewardService.ClaimResult.UNKNOWN_PROFILE||rewardResult==RelationshipRewardService.ClaimResult.NOT_ELIGIBLE){
                return new Result(Status.REWARD_FAILED,cycle,definition.messageKey());
            }
        }
        for(var delta:definition.progressionDeltas().entrySet())relationships.addProgression(player.getUuid(),pokemonId,delta.getKey(),delta.getValue());
        state.setFlag(flag,Long.toString(cycle)); relationships.touch();
