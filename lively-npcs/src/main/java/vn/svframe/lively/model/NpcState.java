...
public final class NpcState {
    ...
    public void rename(String value) { synchronized (stateLock) { name = Objects.requireNonNull(value); revision.incrementAndGet(); } }
    public void setRole(String value) { synchronized (stateLock) { role = Objects.requireNonNull(value); revision.incrementAndGet(); } }
    public void setTrait(String key, double value) { synchronized (stateLock) { traits.put(normalize(key), clamp01(value)); revision.incrementAndGet(); } }
    public void setNeed(String key, double value) { synchronized (stateLock) { needs.put(normalize(key), clamp01(value)); revision.incrementAndGet(); } }
    ...
    public void updateRelationship(UUID subject, double trustDelta, double affinityDelta, double suspicionDelta, double fearDelta) {
        synchronized (stateLock) {
            ...
            relationships.put(subject, new NpcSnapshot.RelationshipView(subject,
                    base.trust() + trustDelta, base.affinity() + affinityDelta, base.suspicion() + suspicionDelta,
                    base.fear() + fearDelta, base.interactions() + 1L));
            revision.incrementAndGet();
        }
    }
        ...
        synchronized (stateLock) {
            ...
            while (memories.size() > memoryLimit) {
                NpcSnapshot.MemoryView least = memories.stream()
                        .min((a, b) -> Double.compare(a.importance(), b.importance())).orElse(memories.peekFirst());
    ...
    /**
     * Removes memories whose recall signal has decayed below a threshold, while preserving permanent memories and
     * a recent floor so a quiet NPC never wakes up with an empty personal history. Safe to run off the server thread.
     */
    public int consolidateMemories(MemoryPolicy policy, Instant now, double minScore, int minimumRecent) {
        Objects.requireNonNull(policy); Objects.requireNonNull(now);
        synchronized (stateLock) {
            int floor = Math.max(0, Math.min(memoryLimit, minimumRecent));
            if (memories.size() <= floor) return 0;
            Set<UUID> recent = new HashSet<>();
            var descending = memories.descendingIterator();
            while (descending.hasNext() && recent.size() < floor) recent.add(descending.next().id());
            int before = memories.size();
            memories.removeIf(memory -> !recent.contains(memory.id()) && !policy.permanent(memory)
                    && policy.recallScore(memory, now) < Math.max(0D, Math.min(1D, minScore)));
    ...
    public NpcSnapshot snapshot(int memoryWindow) {
        synchronized (stateLock) {
            ...
            return new NpcSnapshot(id, revision.get(), Instant.now(), name, role, Map.copyOf(traits), Map.copyOf(needs),
                    Map.copyOf(beliefs), Map.copyOf(relationships), recentMemoriesLocked(safeWindow));
        }
    }
    ...
    public void importData(StateData data) {
        ...
        synchronized (stateLock) { applyDataLocked(data); }
    }

    public boolean importDataIfUnmodified(StateData data) {
        if (!id.equals(data.id())) throw new IllegalArgumentException("npc id mismatch");
        synchronized (stateLock) {
            if (revision.get() != 0L) return false;
            applyDataLocked(data);
            return true;
        }
    }

    private void applyDataLocked(StateData data) {
    ...
    private List<NpcSnapshot.MemoryView> recentMemoriesLocked(int limit) {
        ...
        var iterator = memories.descendingIterator();
        while (iterator.hasNext() && result.size() < limit) result.add(iterator.next());
        return List.copyOf(result);
    }

    public record StateData(UUID id, long revision, String name, String role, Map<String, Double> traits,
                            Map<String, Double> needs, Map<String, NpcSnapshot.BeliefView> beliefs,
                            Map<UUID, NpcSnapshot.RelationshipView> relationships, List<NpcSnapshot.MemoryView> memories) {
        public StateData {
