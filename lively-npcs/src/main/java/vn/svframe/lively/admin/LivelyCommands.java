...
public final class LivelyCommands {
        ...
        LiteralArgumentBuilder<ServerCommandSource> structure = literal("structure")
                ...
                .then(literal("info")
                        .then(argument("id", StringArgumentType.word())
                                .executes(ctx -> structureInfo(ctx.getSource(), StringArgumentType.getString(ctx, "id")))));
        structure.then(literal("create")
                .then(argument("id", StringArgumentType.word())
                        .then(argument("type", StringArgumentType.word())
                                .executes(ctx -> createStructure(ctx.getSource(),
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> worldCommands() {
        LiteralArgumentBuilder<ServerCommandSource> event = literal("event")
                ...
                .then(literal("stop")
                        .then(argument("id", StringArgumentType.word())
                                .executes(ctx -> stopEvent(ctx.getSource(), StringArgumentType.getString(ctx, "id")))));
        event.then(literal("start")
    ...
    private static int status(ServerCommandSource source) {
        source.sendFeedback(() -> Text.literal("Lively: " + LivelyApi.admin().status()), false);
        return 1;
    }
    ...
    private static int createStructure(ServerCommandSource source, String id, String type) {
        ...
        BlockPos a = POS1.get(player);
        BlockPos b = POS2.get(player);
        if (a == null || b == null) {
            source.sendError(Text.literal("Set /lively pos1 and /lively pos2 first"));
            return 0;
        }
        String world = source.getWorld().getRegistryKey().getValue().toString();
        var bounds = new SemanticStructureRegistry.Bounds(
                world,
                Math.min(a.getX(), b.getX()), Math.min(a.getY(), b.getY()), Math.min(a.getZ(), b.getZ()),
                Math.max(a.getX(), b.getX()), Math.max(a.getY(), b.getY()), Math.max(a.getZ(), b.getZ()));
        try {
            LivelyApi.admin().createStructure(id, type, bounds, Set.of(), Map.of(), null, null);
            source.sendFeedback(() -> Text.literal("Created structure " + id + " (" + type + ")"), false);
            return 1;
        } catch (RuntimeException ex) {
            source.sendError(Text.literal("Cannot create structure: " + ex.getMessage()));
            return 0;
        }
    }
    ...
    private static int listStructures(ServerCommandSource source) {
        ...
        source.sendFeedback(() -> Text.literal("Structures (" + values.size() + "): " +
                values.stream().map(SemanticStructureRegistry.Structure::id).sorted().toList()), false);
        return 1;
    }
    ...
    private static int structureInfo(ServerCommandSource source, String id) {
        ...
        if (structure.isEmpty()) {
            source.sendError(Text.literal("Unknown structure: " + id));
            return 0;
        }
    ...
    private static int listEvents(ServerCommandSource source) {
        ...
        source.sendFeedback(() -> Text.literal("Active events (" + events.size() + "): " +
                events.stream().map(e -> e.id() + ":" + e.seed()).toList()), false);
        return 1;
    }

    private static int stopEvent(ServerCommandSource source, String id) {
        try {
            var cancelled = LivelyApi.events().cancel(UUID.fromString(id));
            if (cancelled.isEmpty()) {
                source.sendError(Text.literal("Unknown active event"));
                return 0;
            }
            source.sendFeedback(() -> Text.literal("Cancelled event " + id), false);
            return 1;
        } catch (IllegalArgumentException ex) {
            source.sendError(Text.literal("Invalid event UUID"));
            return 0;
        }
    }
    ...
    private static int startEvent(ServerCommandSource source, String category, String seed, int minutes, double intensity) {
        try {
            WorldEventEngine.Category value = WorldEventEngine.Category.valueOf(category.toUpperCase(java.util.Locale.ROOT));
            var event = LivelyApi.admin().startEvent(value, seed, null, Set.of(), intensity, Duration.ofMinutes(minutes), Map.of("source", "admin"));
            if (event.isEmpty()) {
                source.sendError(Text.literal("Event rejected by policy"));
                return 0;
            }
            source.sendFeedback(() -> Text.literal("Started event " + event.get().id() + " seed=" + seed), false);
            return 1;
        } catch (IllegalArgumentException ex) {
            source.sendError(Text.literal("Unknown category: " + category));
            return 0;
        }
    }
}
