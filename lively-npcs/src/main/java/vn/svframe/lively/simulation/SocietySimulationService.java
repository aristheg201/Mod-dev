...
/**
 * Bounded society pulse tying routine, leisure, romance, gambling, debt and crime together.
 * Law enforcement dispatch/arrest is owned by the evidence-driven justice service, never by hidden perpetrator truth.
 */
public final class SocietySimulationService {
    public enum Activity { WORK, HOME, SLEEP, EAT, DRINK, GAMBLE, AFFECTION, PATROL, DEBT_COLLECTION, CRIME, CUSTODY, IDLE }
    ...
    public void tick(long tick) {
        ...
        lastPulse = tick;
        SocietyApi.debts().accrue(Instant.now(), Duration.ofMinutes(20));
        ...
        for (int n = 0; n < count; n++) {
            ...
            if (definition != null && state != null) process(tick, definition, state, economy, byKind, partners);
        }
    ...
    private void process(long tick, NpcDefinition definition, NpcState state, EconomyEngine.Snapshot economy,
        ...
        if (SocietyApi.law().activeCustody(actor).isPresent()) {
            setActivity(state, definition.id(), Activity.CUSTODY, Map.of());
            return;
        }

        NpcSnapshot snapshot = state.snapshot(8);
        long dayTime = Math.floorMod(server.getOverworld().getTimeOfDay(), 24_000L);
        boolean police = bool(definition, "society.police") || role(definition, "police", "officer", "cảnh sát", "sheriff", "guard");
        ...
        if (payOwnDebt(state, actor, economy)) return;

        double hunger = snapshot.need("hunger");
        if ((hunger > .72D || (dayTime >= 5_500L && dayTime <= 6_500L)) && eat(tick, definition, state, actor, byKind)) return;
        ...
        SplittableRandom random = new SplittableRandom(definition.id().getMostSignificantBits()
                ^ definition.id().getLeastSignificantBits() ^ (tick / PULSE_TICKS));
        ...
        if (dayTime >= 9_000L && dayTime < 17_500L
                && tick - lastLeisure.getOrDefault(definition.id(), Long.MIN_VALUE / 2L) >= LEISURE_COOLDOWN) {
            double roll = random.nextDouble();
            if (gambling > .45D && roll < gambling * .42D && gamble(tick, definition, state, actor, byKind, random)) return;
            if (alcohol > .40D && roll < Math.min(.85D, gambling * .42D + alcohol * .48D) && drink(tick, definition, state, actor, byKind)) return;
        ...
        if (criminality > .62D && moneyPressure > .58D
                && tick - lastCrime.getOrDefault(definition.id(), Long.MIN_VALUE / 2L) >= CRIME_COOLDOWN
                && random.nextDouble() < criminality * moneyPressure * .28D
                && commitEconomicCrime(tick, definition, state, actor, economy, random)) return;
    ...
    private boolean eat(long tick, NpcDefinition definition, NpcState state, ActorId actor,
    ...
    private boolean drink(long tick, NpcDefinition definition, NpcState state, ActorId actor,
                          Map<String, List<EconomyEngine.Business>> byKind) {
    ...
    private boolean gamble(long tick, NpcDefinition definition, NpcState state, ActorId actor,
        ...
        long balance = wallet(LivelyApi.economy().snapshot(), actor);
        ...
        state.setNeed("financial_stress", result == GamblingEngine.Result.LOSS
                ? Math.min(1D, snapshot.need("financial_stress") + .12D)
    ...
    private boolean payOwnDebt(NpcState state, ActorId actor, EconomyEngine.Snapshot economy) {
        ...
        if (balance <= 0L) {
            state.setNeed("financial_stress", Math.min(1D, state.snapshot(1).need("financial_stress") + .05D));
            return false;
        }
    ...
    private boolean affection(long tick, NpcDefinition definition, NpcState state, ActorId actor, RomanceEngine.Bond bond) {
        ...
        if (partner.kind() != ActorId.Kind.NPC || LivelyApi.npcs().get(partner.uuid()).isEmpty()
                || SocietyApi.law().activeCustody(partner).isPresent()) return false;
    ...
    private boolean commitEconomicCrime(long tick, NpcDefinition definition, NpcState state, ActorId actor,
        ...
        EconomyEngine.Business target = economy.businesses().values().stream()
                .filter(business -> !business.owner().equals(actor) && business.open())
    ...
    private static boolean bool(NpcDefinition definition, String key) { return Boolean.parseBoolean(definition.metadata().getOrDefault(key, "false")); }
