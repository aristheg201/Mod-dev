...
/** 1.0.1 world-session bootstrap for commerce, debt, gambling and causal society behaviour. */
public final class SocietyBootstrap implements ModInitializer {
    ...
    private synchronized void start(MinecraftServer server) {
        ...
        Path config = FabricLoader.getInstance().getConfigDir().resolve("livelynpcs").resolve("economy.properties");
        EconomyRoutingConfig.load(config).forEach(SocietyApi.economies()::route);
        ...
        LivelyNpcs.LOGGER.info("Lively society 1.0.1 session ready: economy routes={}, debtContracts={}, gamblingBets={}",
    ...
    private static SocietyStateStore.Bundle capture() {
        return new SocietyStateStore.Bundle(SocietyApi.debts().snapshot(), SocietyApi.gambling().snapshot());
    }
}
