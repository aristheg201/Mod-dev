...
loom {
    ...
    if (System.getenv("SVARCADE_VISUAL_SMOKE") == "1") {
...
dependencies {
    ...
    // external runtime dependencies, never bundled into the SVArcade JAR.
    ...
    // Fabric-native Placeholder API. JIJ it so SVArcade formatting works without
    ...
    // and break unrelated boss bars (for example NovaRaids). SVArcade's rich client
...
tasks.remapJar {
    archiveFileName.set("SVArcade-fabric-${minecraftVersion}-${version}.jar")
}
...
tasks.register<Test>("verifyPackagedTft") {
    ...
    useJUnitPlatform()
    filter { includeTestsMatching("io.github.aristheg201.svarcade.native.game.tft.TftSetRegistryTest") }
    systemProperty("svarcade.test.packaged", "true")
}
...
val generateUiTextures by tasks.registering(Exec::class) {
    ...
    inputs.files("tools/generate-ui-textures.py", "tools/svarcade_png.py")
