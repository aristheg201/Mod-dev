...
for dirname in ['src','tools']:
 for p in (source/dirname).rglob('*'):
  ...
  rel=str(p.relative_to(source)).replace('SVArcade','SVArcade').replace('svarcade','svarcade')
  dest=variant/rel;dest.parent.mkdir(parents=True,exist_ok=True)
  if p.suffix in ['.kt','.java','.json','.py','.md','.txt','.properties','.csv','.mcmeta']:
   try:
    s=p.read_text(encoding='utf-8').replace('SVArcade','SVArcade').replace('svarcade','svarcade').replace('SVARCADE','SVARCADE')
    for suffix in ['_dbz_','_naruto_','-dbz','-hunter']:
     s=s.replace('svarcade'+suffix,'svarcade'+suffix)
...
for name in ['build.gradle.kts','settings.gradle','gradle.properties']:
 s=(source/name).read_text(encoding='utf-8').replace('SVArcade','SVArcade').replace('svarcade','svarcade').replace('SVARCADE','SVARCADE')
...
for folder,other in [(source,'svarcade'),(variant,'svarcade')]:
