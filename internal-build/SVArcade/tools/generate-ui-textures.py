...
import sys
import zipfile
from svarcade_png import decode, encode

ROOT = Path(__file__).resolve().parents[1]
RESOURCES = ROOT / "src/main/resources"
PREFIX = "assets/svarcade/textures/gui/generated/"
...
def verify(resources, expected):
    errors = []
    if not resources:
        errors.append("no bundled SVArcade PNG assets found")
...
def main(argv=None):
    ...
    source = {p.relative_to(RESOURCES).as_posix(): p.read_bytes() for p in (RESOURCES / "assets/svarcade").rglob("*.png")}
    rendered = {p.relative_to(options.output).as_posix(): p.read_bytes()
                for p in (options.output / "assets/svarcade").rglob("*.png")}
    ...
    if options.jar is not None:
        with zipfile.ZipFile(options.jar) as archive:
            ...
            errors += [f"duplicate packaged asset: {n}" for n, count in Counter(names).items() if count > 1 and n.startswith("assets/svarcade/")]
            packaged = {n: archive.read(n) for n in names if n.startswith("assets/svarcade/") and n.endswith(".png")}
