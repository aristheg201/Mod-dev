...
import io.github.aristheg201.svarcade.native.ArcadeCapabilities
...
object ArcadeAcceptanceHarness {
    ...
    var complete = false
        private set
    fun register() {
        if (System.getenv("SVARCADE_ARCADE_SMOKE") != "1") { complete=true;return }
        ClientTickEvents.END_CLIENT_TICK.register { client ->
            if(complete) return@register
            if(client.overlay != null) return@register
            if(page==0 && screen==null) { client.options.guiScale().set(System.getenv("SVARCADE_ACCEPTANCE_SCALE")?.toIntOrNull() ?: 2);client.resizeDisplay() }
            if(page>=pages.size) {
                complete=true
                ...
                if(System.getenv("SVARCADE_ARCADE_ONLY") == "1") client.stop()
                return@register
            }
            val name=pages[page]
            if(screen == null) {
