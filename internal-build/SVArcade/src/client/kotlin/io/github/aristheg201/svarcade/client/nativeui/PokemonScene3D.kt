...
import io.github.aristheg201.svarcade.client.cobblemon.CobblemonSceneParticleCue
...
import io.github.aristheg201.svarcade.ui.SceneProjection
...
import io.github.aristheg201.svarcade.ui.SceneEffectNode
...
data class PokemonSceneLayout(
    val area: UiRect,val columns: Int,val rows: Int,val originX: Float,val originY: Float,val tileWidth: Int,val tileHeight: Int,val perspective:io.github.aristheg201.svarcade.ui.PerspectiveBoardTransform?=null,
    val boardSurface: io.github.aristheg201.svarcade.ui.SceneInteractionSurface = io.github.aristheg201.svarcade.ui.SceneInteractionSurface("board", io.github.aristheg201.svarcade.ui.SceneVec3(-.5,-.5,0.0), columns.toDouble(), rows.toDouble(), columns, rows)
) {
    fun project(x: Float, y: Float, z: Float = 0f): ScenePoint? {
        if (perspective != null) return perspective.project(io.github.aristheg201.svarcade.ui.SceneVec3(x.toDouble(),y.toDouble(),z.toDouble()))?.let { ScenePoint(it.x,it.y) }
        ...
        if (embedded) {
            EmbeddedSceneRenderer.render(gui,layout,arena) { poses,buffers ->
                ...
                state.activeEffects(now).forEachIndexed { index,effect ->
                    ...
                    SceneEffectsRenderer.render(poses,SceneEffectNode("event:$index",SceneTransform(point),"svarcade:${effect.kind.name.lowercase()}",now-(effect.progress*480).toLong()),now)
                }
