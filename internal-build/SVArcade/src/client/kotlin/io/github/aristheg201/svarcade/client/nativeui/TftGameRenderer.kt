...
import io.github.aristheg201.svarcade.ui.TftLayoutResolver
import io.github.aristheg201.svarcade.ui.SceneTransform
import io.github.aristheg201.svarcade.ui.SceneVec3
import io.github.aristheg201.svarcade.ui.SceneItemModelNode
import io.github.aristheg201.svarcade.ui.SceneTacticianNode
import io.github.aristheg201.svarcade.ui.SceneCameraPreset
import io.github.aristheg201.svarcade.ui.SVArcadeSceneCamera
import io.github.aristheg201.svarcade.ui.SceneCameraFraming
...
class TftUiState {
    ...
    private var cameraStart:SVArcadeSceneCamera?=null
    private var cameraCurrent:SVArcadeSceneCamera?=null
    ...
    fun camera(destination:SceneCameraPreset,now:Long=System.currentTimeMillis()):SceneCameraPreset {
        val target=SVArcadeSceneCamera(destination.position,destination.target,destination.fov,destination.near,destination.far)
...
object TftGameRenderer {
    ...
    fun render(
        ...
        if(phase=="draft"&&draft.isNotEmpty()){
            ...
            gui.drawCenteredString(font,fit(font,tr("gui.svarcade.tft.shared_draft"),header.width-120),header.x+header.width/2,header.y+7,text)
        ...
        resolved.phaseBanner?.let { banner ->
            ...
            if(titleH>0){
                val title = tr(if(hasLoot) "gui.svarcade.tft.loot_ready" else if(fields.bool("bossRound")) "gui.svarcade.tft.boss_round" else "gui.svarcade.tft.pve_round")
    ...
    private fun renderHud(gui: GuiGraphics, font: Font, area: UiRect, fields: JsonObject, phase: String, status: String, density: UiDensity, hooks: Hooks, mouseX: Int, mouseY: Int) {
        ...
        val title = if (density == UiDensity.COMPACT) "$round • Lv.$level • ${goldValue}g • HP $hp" else "${tr("gui.svarcade.game.tft.title")}  •  $round"
        ...
        if (density != UiDensity.COMPACT) {
            val economy = "${goldValue}g  •  HP $hp  •  ${tr("gui.svarcade.tft.interest")}: ${fields.int("lastInterest")}  •  ${tr("gui.svarcade.tft.streak")}: ${if (streak >= 0) "+$streak" else streak}"
            gui.drawString(font, fit(font, economy, area.width - 180), area.x + 58, area.y + 19, muted, false)
        }
        val phaseText = when (phase) { "planning" -> tr("gui.svarcade.tft.planning"); "combat" -> tr("gui.svarcade.tft.combat"); "draft" -> tr("gui.svarcade.tft.draft"); "post" -> tr("gui.svarcade.tft.results"); else -> phase }
        ...
        if (density == UiDensity.WIDE) {
            val semanticStatus = when {
                fields.str("eliminated") == "true" -> tr("gui.svarcade.tft.eliminated")
                phase == "combat" && fields.str("opponent").isNotBlank() -> trf("gui.svarcade.tft.vs", fields.str("opponent"))
                else -> ""
            }
    ...
    private fun renderTraits(
        ...
        gui.drawString(font, tr("gui.svarcade.tft.traits"), rect.x + 7, rect.y + 7, muted, true)
        val allRect=UiRect(rect.right-34,rect.y+3,30,14)
        hooks.control(allRect,tr("gui.svarcade.tft.set"),true){
    ...
    private fun renderSetBrowser(gui:GuiGraphics,font:Font,area:UiRect,ui:TftUiState,hooks:Hooks,mouseX:Int,mouseY:Int){
        ...
        gui.drawString(font,tr("gui.svarcade.tft.set_browser"),panelRect.x+10,panelRect.y+9,text,true)
        hooks.control(UiRect(panelRect.right-30,panelRect.y+5,22,18),"×",true){ui.setBrowserOpen=false}
        val tabY=panelRect.y+30
        hooks.control(UiRect(panelRect.x+10,tabY,70,18),tr("gui.svarcade.tft.units"),true){
            ui.setBrowserTab="UNITS";ui.setBrowserPage=0
        }
        hooks.control(UiRect(panelRect.x+84,tabY,70,18),tr("gui.svarcade.tft.traits"),true){
        ...
        if(ui.setBrowserTab=="TRAITS"){
            ...
            entries.drop(ui.setBrowserPage*pageSize).take(pageSize).forEachIndexed{offset,trait->
                ...
                if(r.contains(mouseX.toDouble(),mouseY.toDouble())){
                    ui.offerTooltip(TftHoverTooltip(trait.name,tr("gui.svarcade.tft.tooltip.trait"),
    ...
    private fun renderPlayers(gui: GuiGraphics, font: Font, rect: UiRect, players: List<PlayerLine>, hooks: Hooks) {
        gui.fill(rect.x, rect.y, rect.right, rect.y+20,0xC0101B1F.toInt())
        gui.drawString(font, tr("gui.svarcade.tft.players"), rect.x + 7, rect.y + 7, muted, true)
        var y = rect.y + 23
    ...
    private fun renderBoard(
        ...
        if(showBoardLabel) {
            gui.drawCenteredString(
                font,
                if (phase == "combat") tr("gui.svarcade.tft.enemy_board") else tr("gui.svarcade.tft.enemy_side"),
                rect.x + rect.width / 2,
                rect.y + 3,
                muted
            )
    ...
    private fun renderFooter(gui: GuiGraphics, font: Font, rect: UiRect, density: UiDensity, view: JsonObject, fields: JsonObject, bench: List<BenchToken>, canEdit: Boolean, canBuy:Boolean,canSell:Boolean,ui: TftUiState, hooks: Hooks, mouseX: Int, mouseY: Int) {
        ...
        hooks.control(UiRect(buttonX, rect.y + 2, buttonCell - 2, benchH - 4), tr("gui.svarcade.tft.reroll"), view.actionEnabled("refresh")) { hooks.action("refresh", emptyMap()) }
        hooks.control(UiRect(buttonX + buttonCell, rect.y + 2, buttonCell - 2, benchH - 4), tr("gui.svarcade.tft.buy_xp"), view.actionEnabled("buy_xp")) { hooks.action("buy_xp", emptyMap()) }
        val locked = fields.bool("shopLocked")
        hooks.control(UiRect(buttonX + buttonCell * 2, rect.y + 2, buttonW - buttonCell * 2 - 2, benchH - 4), tr(if (locked) "gui.svarcade.tft.shop_locked" else "gui.svarcade.tft.shop_lock"), canBuy) {
        ...
        if (cards != null && cards.size() > 0) {
            ...
            repeat(min(shopSlots, cards.size())) { index ->
                ...
                if (cardRect.contains(mouseX.toDouble(), mouseY.toDouble())) {
                    ...
                    ui.offerTooltip(if (elite && tooltip != null) tooltip.copy(lines = tooltip.lines + tr("gui.svarcade.tft.elite_bonus")) else tooltip)
                }
        ...
        if (canSell && ui.selectedOrigin != null && ui.selectedIndex != null) {
            ...
            hooks.control(sellRect, tr("gui.svarcade.tft.sell"), true) {
    ...
    private fun renderItemRail(
        ...
        gui.drawString(font,fit(font,tr("gui.svarcade.tft.items"),rect.width-12),rect.x+7,rect.y+6,text,true)
        ...
        items.drop(first).take(pageSize).forEachIndexed { offset,item ->
            ...
            if(itemRect.contains(mouseX.toDouble(),mouseY.toDouble()))
                ui.offerTooltip(TftHoverTooltip(ui.itemName(item),tr("gui.svarcade.tft.tooltip.item"),ui.itemDetails(item),gold))
    ...
    private fun renderAugmentHud(gui: GuiGraphics, font: Font, board: UiRect, fields: JsonObject,
        ...
        val feedback = if (ui.acquisitionVisible(fields.long("acquisitionSerial")) && event.size >= 3) {
            if (event[0].endsWith("AUTO_SELL")) trf("gui.svarcade.tft.auto_sold", humanize(event[1]), event[2]) +
                event.getOrNull(3)?.takeIf(String::isNotBlank)?.let { " · " + trf("gui.svarcade.tft.item_received", it.split(',').joinToString { id -> ui.itemName(id) }) }.orEmpty()
            else trf("gui.svarcade.tft.star_up", humanize(event[1]), event[2])
        } else if (pending > 0) trf("gui.svarcade.tft.pending_items", pending) else ""
        if (feedback.isNotBlank()) {
            ...
            if (region.contains(mouseX.toDouble(), mouseY.toDouble())) ui.offerTooltip(TftHoverTooltip(feedback, lines = listOf(trf("gui.svarcade.tft.pending_items", pending)), accent = gold))
        }
        ...
        if (fields.str("scouting") == "true") {
            hooks.control(UiRect(x + 46, y, 48, 14), tr("gui.svarcade.tft.home"), true) { hooks.action("scout", mapOf("target" to "home")) }
        }
    }
    ...
    private fun renderAugmentOverlay(
        ...
        gui.drawCenteredString(font,tr("gui.svarcade.tft.choose_augment"),root.x+root.width/2,root.y+11,text)
        ...
        visible.forEachIndexed{index,choice->
            ...
            NativeControlRenderer.draw(gui,font,UiRect(base.x+8,base.bottom-27,base.width-16,20),
                tr("gui.svarcade.tft.augment.confirm"),mouseX,mouseY,active=hovered)
    ...
    private fun renderCarouselScene(gui: GuiGraphics, font: Font, board: UiRect, offers: List<DraftOffer>, fields: JsonObject,
        ...
        gui.drawCenteredString(font, fit(font,tr(if(unlocked) "gui.svarcade.tft.carousel.move" else "gui.svarcade.tft.carousel.locked"),board.width-8), board.x + board.width / 2, board.bottom - 18, muted)
    }
    ...
    private fun unitTooltip(
        ...
        if (traitNames.isNotEmpty()) lines += "${tr("gui.svarcade.tft.tooltip.traits")}: ${traitNames.joinToString(" • ")}"
        lines += "${tr("gui.svarcade.tft.tooltip.skill")}: ${info.abilityName.ifBlank { humanize(unitId) }}"
        val abilityParts = buildList {
            add("${tr("gui.svarcade.tft.tooltip.target")}: ${humanize(info.abilityTarget)}")
            if (info.damage > 0) add("${tr("gui.svarcade.tft.tooltip.damage")}: ${info.damage} ${humanize(info.damageType)}")
            if (info.heal > 0) add("${tr("gui.svarcade.tft.tooltip.heal")}: ${info.heal}")
            if (info.shield > 0) add("${tr("gui.svarcade.tft.tooltip.shield")}: ${info.shield}")
            if (info.radius > 0) add("${tr("gui.svarcade.tft.tooltip.radius")}: ${info.radius}")
            if (info.stunMs > 0) add("${tr("gui.svarcade.tft.tooltip.stun")}: ${"%.1f".format(java.util.Locale.ROOT, info.stunMs / 1000.0)}s")
            if (info.dash > 0) add("${tr("gui.svarcade.tft.tooltip.dash")}: ${info.dash}")
            if (info.effects.isNotBlank()) add(humanizeEffects(info.effects))
        }
        if (abilityParts.isNotEmpty()) lines += abilityParts.joinToString(" • ")
        lines += "${tr("gui.svarcade.tft.tooltip.stats")}: HP ${info.hp} • AD ${info.attackDamage} • DEF ${info.defense}/${info.specialDefense} • AS ${"%.2f".format(java.util.Locale.ROOT, info.attackSpeed)} • RNG ${info.range}"
        val manaLine = if (maxMana > 0) "$mana/$maxMana" else "${info.manaStart}/${info.manaMax}"
        lines += "${tr("gui.svarcade.tft.tooltip.mana")}: $manaLine"
        if (hp >= 0 && maxHp > 0) lines += "HP: $hp/$maxHp"
        if (items.isNotEmpty()) lines += "${tr("gui.svarcade.tft.tooltip.items")}: ${items.joinToString(" • ") { humanize(it.substringAfter(':')) }}"
    ...
    private fun traitTooltip(ui: TftUiState, trait: TraitLine): TftHoverTooltip {
        ...
        val progress = buildString {
            append(tr("gui.svarcade.tft.tooltip.active")).append(": ").append(trait.count)
            if (trait.active > 0) append(" • ").append(tr("gui.svarcade.tft.tooltip.tier")).append(" ").append(trait.active)
            if (trait.next > 0) append(" • ").append(tr("gui.svarcade.tft.tooltip.next")).append(": ").append(trait.next)
        }
        lines += progress
        ...
        if (tiers.isNotEmpty()) {
            tiers.forEach { tier ->
                ...
                if (tier.teamEffects.isNotBlank()) lines += "  ${tr("gui.svarcade.tft.tooltip.team")}: ${humanizeEffects(tier.teamEffects)}"
            }
        } else if (trait.description.isNotBlank()) lines += trait.description
        return TftHoverTooltip(
            title = info?.name ?: trait.name,
            subtitle = "${trait.count} ${tr("gui.svarcade.tft.tooltip.units")}",
            lines = lines,
