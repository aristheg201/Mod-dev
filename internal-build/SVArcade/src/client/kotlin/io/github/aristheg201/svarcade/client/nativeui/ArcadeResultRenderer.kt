...
object ArcadeResultRenderer {
    ...
    fun render(gui:GuiGraphics,font:Font,area:UiRect,view:JsonObject,hooks:Hooks,backdrop:(UiRect)->Unit) {
        ...
        val outcomeText=trOr("gui.svarcade.result."+outcome,outcome.replaceFirstChar(Char::uppercase))
        ...
        gui.drawCenteredString(font,trOr("gui.svarcade.result.reason."+reason,tr("gui.svarcade.result.complete")),scene.x+scene.width/2,scene.bottom-18,0xFFF2F6F4.toInt())
        ...
        renderSection(gui,font,UiRect(area.x+margin+gap,panelTop+5,columnW,panelH-10),tr("gui.svarcade.result.stats"),result.getAsJsonArray("stats"),"metric",accent)
        renderSection(gui,font,UiRect(area.x+margin+gap*2+columnW,panelTop+5,columnW,panelH-10),tr("gui.svarcade.result.rewards"),result.getAsJsonArray("rewards"),"reward",0xFFE2BE62.toInt())
        renderSection(gui,font,UiRect(area.x+margin+gap*3+columnW*2,panelTop+5,columnW,panelH-10),tr("gui.svarcade.result.progress"),result.getAsJsonArray("progression"),"progress",0xFF4CC7B2.toInt())

        val y=area.bottom-controlsH+2
        val buttonGap=4
        val buttonW=((area.width-buttonGap*4)/3).coerceAtLeast(52)
        hooks.control(UiRect(area.x+buttonGap,y,buttonW,20),tr("gui.svarcade.result.continue"),true,hooks.continueAction)
        hooks.control(UiRect(area.x+buttonGap*2+buttonW,y,buttonW,20),tr("gui.svarcade.result.rematch"),result.bool("canRematch",true),hooks.rematch)
        hooks.control(UiRect(area.x+buttonGap*3+buttonW*2,y,buttonW,20),tr("gui.svarcade.result.exit"),true,hooks.exit)
    }
    ...
    private fun renderSection(gui:GuiGraphics,font:Font,rect:UiRect,title:String,lines:JsonArray?,kind:String,accent:Int){
        ...
        if(entries.isEmpty()){
            gui.drawString(font,tr("gui.svarcade.result.none"),rect.x+8,rect.y+21,0xFF91A6A1.toInt(),false)
            return
        }
        var y=rect.y+21
        entries.take(4).forEach{line->
            ...
            val label=trOr("gui.svarcade.result."+kind+"."+key,humanize(key))
