...
import io.github.aristheg201.svarcade.client.cobblemon.CobblemonWikiProvider
import io.github.aristheg201.svarcade.client.cobblemon.ClientPokemonRuntimeInfo
import io.github.aristheg201.svarcade.client.cobblemon.PokemonInfoProvider
import io.github.aristheg201.svarcade.client.cobblemon.PokemonModelRenderer
import io.github.aristheg201.svarcade.client.editor.HubEditorScreen
import io.github.aristheg201.svarcade.client.gui.HubScreen
import io.github.aristheg201.svarcade.client.nativeui.MinecraftArenaRegistry
import io.github.aristheg201.svarcade.client.nativeui.NativeGameVisualRegistry
import io.github.aristheg201.svarcade.client.nativeui.VanillaCompanionModelRenderer
import io.github.aristheg201.svarcade.native.network.NativeResumeTftC2S
import io.github.aristheg201.svarcade.client.render.GeneratedBackgroundRenderer
import io.github.aristheg201.svarcade.client.render.MiniMessageText
import io.github.aristheg201.svarcade.content.HUB_PROTOCOL_VERSION
import io.github.aristheg201.svarcade.network.*
...
object SVArcadeClient : ClientModInitializer {
    ...
    private val snapshotExecutor = Executors.newSingleThreadExecutor { task -> Thread(task, "SVArcade-Client-IO").apply { isDaemon = true } }

    override fun onInitializeClient() {
        openHubKey = KeyBindingHelper.registerKeyBinding(KeyMapping("key.svarcade.open", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_H, "key.categories.svarcade"))
        ResourceManagerHelper.get(PackType.CLIENT_RESOURCES).registerReloadListener(object : SimpleSynchronousResourceReloadListener {
            override fun getFabricId(): ResourceLocation = ResourceLocation.fromNamespaceAndPath("svarcade", "client_render_caches")
        ...
        ClientPlayNetworking.registerGlobalReceiver(HubHelloS2C.TYPE) { payload, context -> context.client().execute {
            ...
            if(payload.protocol!=HUB_PROTOCOL_VERSION)context.player().sendSystemMessage(Component.literal("SVArcade protocol không tương thích: client=$HUB_PROTOCOL_VERSION, server=${payload.protocol}"))else if(cachedRevision==payload.revision)openPendingIfReady()
        }}
        ClientPlayNetworking.registerGlobalReceiver(HubSnapshotChunkS2C.TYPE){payload,context->context.client().execute{val encoded=ClientHubState.acceptSnapshotChunk(payload.transferId,payload.editor,payload.index,payload.total,payload.chunk)?:return@execute;snapshotExecutor.execute decode@{val decoded=runCatching{ClientHubState.decodeSnapshot(encoded)}.getOrElse{error->context.client().execute{ClientHubState.lastEditorMessage="Không thể đọc snapshot SVArcade: ${error.message}"};return@decode};context.client().execute apply@{if(!ClientHubState.applySnapshot(decoded,payload.revision,payload.editor))return@apply;if(payload.editor&&pendingEditor){pendingEditor=false;Minecraft.getInstance().setScreen(HubEditorScreen())}else if(!payload.editor)openPendingIfReady()}}}}
