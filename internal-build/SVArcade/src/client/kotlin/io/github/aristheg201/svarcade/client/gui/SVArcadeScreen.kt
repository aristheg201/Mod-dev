...
/**
 * Base screen for every SVArcade UI.
 *
 ...
 * darkening layer. SVArcade already draws its own full-screen pixel background, so
 * allowing the vanilla pass here would blur/darken the Hub itself every frame.
 */
abstract class SVArcadeScreen(title: Component) : Screen(title) {
    final override fun renderBackground(gui: GuiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
        // Intentionally empty: SVArcade screens own their complete background pass.
    }
