...
import io.github.aristheg201.svarcade.client.CommandViewProvider
import io.github.aristheg201.svarcade.client.api.SVArcadeClientApi
...
object HubDynamicGridRenderer {
    ...
    private fun entries(provider: String, content: HubContent): List<Entry> = when (provider) {
        ...
        "svarcade:commands", "environment:commands" -> CommandViewProvider.all().map { Entry("/${it.name}", it.syntaxes.firstOrNull().orEmpty(), it.route) }
        else -> SVArcadeClientApi.grid(provider, content)?.map { Entry(it.title, it.subtitle, it.route) }.orEmpty()
    }
