...
import io.github.aristheg201.svarcade.native.network.NativeJsonDelta
import io.github.aristheg201.svarcade.native.network.NativeJsonPatch
import io.github.aristheg201.svarcade.client.gui.SVArcadeScreen
import io.github.aristheg201.svarcade.native.network.NativeCloseC2S
import io.github.aristheg201.svarcade.native.network.NativeIntentC2S
import io.github.aristheg201.svarcade.ui.NativeLayout
import io.github.aristheg201.svarcade.ui.ScrollbarLayout
import io.github.aristheg201.svarcade.ui.ScrollbarMetrics
...
class NativePlatformScreen(
    ...
    val viewId: String
) : SVArcadeScreen(Component.translatable("screen.svarcade.native")) {
    ...
    private val modVersion: String = FabricLoader.getInstance().getModContainer("svarcade").map { it.metadata.version.friendlyString }.orElse("?")
    ...
    private fun drawNavigation(gui: GuiGraphics, layout: NativeLayout, mouseX: Int, mouseY: Int) {
        val entries = listOf(
            "arcade" to "gui.svarcade.nav.home", "gacha" to "gui.svarcade.nav.gacha", "skins" to "gui.svarcade.nav.skins",
            "store" to "gui.svarcade.nav.store", "companions" to "gui.svarcade.nav.arena", "wallet" to "gui.svarcade.nav.wallet"
        )
        if (layout.verticalNavigation) {
            ...
            addControl(UiRect(layout.navigation.x + 5, layout.navigation.bottom - 39, layout.navigation.width - 10, 32), tr("gui.svarcade.nav.wallet"), mouseX, mouseY, "wallet", module == "wallet") { open("wallet") }
        } else {
    ...
    private fun renderDashboard(gui: GuiGraphics, layout: NativeLayout, mouseX: Int, mouseY: Int) {
        ...
        gui.drawString(font, tr("gui.svarcade.dashboard.title"), area.x, area.y + 1, text, true)
        val backend = if (state.bool("skinBackendReady")) tr("gui.svarcade.status.online") else tr("gui.svarcade.status.offline")
        gui.drawString(font, "SkiesSkins $backend  •  ${state.num("ownedSkins")}/${state.num("skinTotal")}", area.x, area.y + 15, muted, false)
        val cards = listOf(
            Triple("gacha", tr("gui.svarcade.nav.gacha"), "${wallet?.num("ticket") ?: 0} ${tr("gui.svarcade.ticket")}"),
            Triple("skins", tr("gui.svarcade.nav.skins"), "${state.num("ownedSkins")} ${tr("gui.svarcade.owned")}"),
            Triple("arcade", tr("gui.svarcade.nav.arcade"), "${state.num("gameCount")} ${tr("gui.svarcade.games")}"),
            Triple("companions", tr("gui.svarcade.nav.arena"), tr("gui.svarcade.arena.ready")),
            Triple("store", tr("gui.svarcade.nav.store"), tr("gui.svarcade.store.preview")),
            Triple("wallet", tr("gui.svarcade.nav.wallet"), tr("gui.svarcade.nav.wallet"))
        )
    ...
    private fun renderGacha(gui:GuiGraphics,layout:NativeLayout,mouseX:Int,mouseY:Int){
        ...
        gui.drawString(font,"${tr("gui.svarcade.ticket")}: ${wallet?.num("ticket") ?: 0}",area.x,area.y,gold,true)
        ...
        val banners=listOf("hunter" to tr("gui.svarcade.gacha.hunter"),"beast" to tr("gui.svarcade.gacha.beast"))
        ...
        banners.forEachIndexed{index,(id,title)->
            ...
            gui.drawString(font,tr("gui.svarcade.gacha.cost"),rect.x+48,rect.y+29,muted,false)
            addControl(
                UiRect(rect.right-74,rect.bottom-23,66,17),
                if(rolling)tr("gui.svarcade.gacha.rolling") else tr("gui.svarcade.roll"),
                mouseX,
                mouseY,
                enabled=!rolling
            ){
    ...
    private fun renderSkins(gui:GuiGraphics,layout:NativeLayout,mouseX:Int,mouseY:Int){
        ...
        gui.drawString(font,"${tr("gui.svarcade.owned")}: ${state.num("ownedCount")}  •  ${page+1}/$pages",area.x,area.y,muted,false)
        val tabs=listOf("all" to "gui.svarcade.filter.all","dbz" to "DBZ","naruto" to "Naruto","pokelegends" to "PokeLegends")
        val tabGap=3
        ...
        if(selected!=null){
            ...
            gui.drawCenteredString(font,tr("gui.svarcade.not_found"),stage.x+stage.width/2,stage.bottom-18,muted)
        }
        ...
        entries.forEachIndexed{i,e->
            ...
            val subtitle=if(owned)tr("gui.svarcade.owned") else e.str("rarity")
    ...
    private fun renderCompanions(gui:GuiGraphics,layout:NativeLayout,mouseX:Int,mouseY:Int){
        ...
        gui.drawString(font,tr("gui.svarcade.arena.title"),area.x,area.y,text,true)
        if(arena!=null){
        ...
        gui.drawString(font,tr("gui.svarcade.arena.pick"),area.x,area.y+14,muted,false)
        ...
        if(selected!=null){
            ...
            addControl(UiRect(area.x,y,(area.width-6)/2,22),tr("gui.svarcade.arena.fight"),mouseX,mouseY,enabled=selected==serverSelected){
                intent("arena_start",JsonObject())
            }
            addControl(UiRect(area.x+(area.width+6)/2,y,(area.width-6)/2,22),tr("gui.svarcade.arena.dismiss"),mouseX,mouseY){
    ...
    private fun renderCompanionArena(gui:GuiGraphics,area:UiRect,arena:JsonObject,mouseX:Int,mouseY:Int){
        ...
        gui.drawString(font,trf("gui.svarcade.arena.energy",arena.num("playerEnergy")),playerRect.x+7,playerRect.bottom-27,gold,false)

        val finished=arena.bool("finished")
        val logY=modelBottom+7
        val arenaStatus=when{
            finished&&arena.str("result")=="Victory"->tr("gui.svarcade.arena.victory")
            finished->tr("gui.svarcade.arena.defeat")
            else->trf("gui.svarcade.arena.turn",arena.num("turn"))
        }
        ...
        if(finished){
            addControl(UiRect(area.x,controlsY,area.width,21),tr("gui.svarcade.arena.rematch"),mouseX,mouseY){
                intent("arena_start",JsonObject())
            }
        }else{
            val gap=4
            val w=(area.width-gap*2)/3
            addControl(UiRect(area.x,controlsY,w,21),tr("gui.svarcade.arena.attack"),mouseX,mouseY){
                intent("arena_act",json("move" to "attack"))
            }
            addControl(UiRect(area.x+w+gap,controlsY,w,21),tr("gui.svarcade.arena.skill"),mouseX,mouseY,enabled=arena.num("playerEnergy")>=2){
                intent("arena_act",json("move" to "skill"))
            }
            addControl(UiRect(area.x+(w+gap)*2,controlsY,w,21),tr("gui.svarcade.arena.guard"),mouseX,mouseY){
    ...
    private fun renderGame(gui:GuiGraphics,layout:NativeLayout,mouseX:Int,mouseY:Int){
        ...
        val area=layout.content.inset(8)
        gui.drawString(font,gameTitleFor(gameId,view.str("title",tr("gui.svarcade.game"))),area.x,area.y,text,true)
        ...
        if(gameId=="tower_defense" && selectedCell!=null){
            ...
            if(token.startsWith("tower:")){
                ...
                val w=54
                addControl(UiRect(area.right-w*2-8,y,w,20),tr("gui.svarcade.td.upgrade"),mouseX,mouseY){
                    gameAct("upgrade",mapOf("slot" to selectedCell.toString()))
                }
                addControl(UiRect(area.right-w-4,y,w,20),tr("gui.svarcade.td.sell"),mouseX,mouseY){
        ...
        if(cards!=null&&cards.size()>0&&!cardTable){
            ...
            repeat(count){index->
                ...
                val cardSubtitle=if(gameId=="tower_defense"){
                    ...
                    trf("gui.svarcade.td.card_stats",card.num("cost"),meta.str("damage"),meta.str("range"))
        ...
        if(gameId=="uno" && unoPendingCard!=null){
            ...
            colors.forEachIndexed{index,color->
                ...
                addControl(rect,tr("gui.svarcade.uno.$color"),mouseX,mouseY){
    ...
    private fun intent(action:String,data:JsonObject) {
        if (System.getenv("SVARCADE_ARCADE_SMOKE") == "1" && acceptanceIntent != null) acceptanceIntent!!.invoke(action,data)
    ...
    private fun gameTitleFor(gameId:String,fallback:String):String =
        trOr("gui.svarcade.game." + gameId + ".title", fallback)
    ...
    private fun localizedGameAction(view:JsonObject,action:JsonObject):String {
        ...
        return when {
            id=="move" && view.str("gameId")=="ludo" -> {
                ...
                if(piece!=null)trf("gui.svarcade.ludo.move_piece",piece) else trOr("gui.svarcade.action.move",action.str("label",id))
            }
            id=="start_wave" -> trf("gui.svarcade.action.start_wave",fields.num("wave")+1)
            else -> trOr("gui.svarcade.action." + id,action.str("label",id))
        }
    }
    ...
    private fun localizedGameStatus(view:JsonObject):String {
        if(view.bool("finished")){
            ...
            return if(winner.isNotBlank())trf("gui.svarcade.game.finished_winner",winner) else tr("gui.svarcade.game.finished")
        }
        val gameId=view.str("gameId")
        val phase=view.str("phase")
        val fields=view.getAsJsonObject("fields")?:JsonObject()
        return when(gameId){
            "chess" -> if(view.str("status")=="Check")tr("gui.svarcade.chess.check") else trf("gui.svarcade.game.turn",view.str("turn"))
            "xiangqi" -> if(view.str("status")=="Chiếu tướng")tr("gui.svarcade.xiangqi.check") else trf("gui.svarcade.game.turn",view.str("turn"))
            "ludo" -> if(phase=="roll")tr("gui.svarcade.ludo.roll_wait") else trf("gui.svarcade.ludo.rolled",fields.num("rolled"))
            "uno" -> {
                val color=fields.str("activeColor")
                trf("gui.svarcade.uno.status",trOr("gui.svarcade.uno." + color,color),fields.str("top"))
            }
            "pokecards" -> if(phase=="waiting")tr("gui.svarcade.pokecards.waiting") else tr("gui.svarcade.pokecards.pick")
            "tower_defense" -> if(fields.bool("running"))trf("gui.svarcade.td.wave_running",fields.num("wave")) else trf("gui.svarcade.td.prepare",fields.num("wave")+1)
    ...
    private fun moduleTitle(value:String)=tr("gui.svarcade.module.$value")
