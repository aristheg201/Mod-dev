...
import io.github.aristheg201.svarcade.ui.TftExclusiveLayout
import io.github.aristheg201.svarcade.ui.TftPresentationPolicy
...
object TftExclusivePresentation {
    ...
    private val logger = LoggerFactory.getLogger("SVArcade/Presentation")
    ...
    fun renderIfOwned(
        ...
        val title = I18n.get(if (owner == TftPresentationPolicy.Owner.BOSS_INTRO) "gui.svarcade.tft.boss_round" else "gui.svarcade.tft.loot_ready")
        ...
        hooks.control(layout.continueButton, I18n.get("gui.svarcade.result.continue"), true) {
    ...
    private fun renderLoot(
        ...
        loot.drop(state.page * pageSize).take(pageSize).forEachIndexed { index, token ->
            ...
            val label = when {
                ...
                token.startsWith("free_reroll:") -> "${I18n.get("gui.svarcade.tft.reroll")} +${token.substringAfter(':')}"
                token.startsWith("unit:") -> ui.unitInfo(token.substringAfter(':'))?.name ?: I18n.get("gui.svarcade.result.reward.loot_count")
                !stack.isEmpty -> ui.itemName(id)
                else -> I18n.get("gui.svarcade.result.reward.loot_count")
            }
