...
import io.github.aristheg201.svarcade.native.game.CardDuelSession
import io.github.aristheg201.svarcade.native.game.ChessSession
import io.github.aristheg201.svarcade.native.game.NativeGameView
import io.github.aristheg201.svarcade.native.game.NativeSeat
import io.github.aristheg201.svarcade.native.game.TowerDefenseDefinitions
import io.github.aristheg201.svarcade.native.game.TowerDefenseSession
import io.github.aristheg201.svarcade.native.game.UnoSession
import io.github.aristheg201.svarcade.native.game.XiangqiSession
...
/**
 * Headless/CI visual acceptance harness for SVArcade.
 *
...
object VisualSmokeHarness {
    private const val ENV = "SVARCADE_VISUAL_SMOKE"
    ...
    fun register() {
        ...
        enabled = true
        System.out.println("[SVArcade Visual Smoke] enabled")
    ...
    private fun tick(client: Minecraft) {
        ...
        if (!ArcadeAcceptanceHarness.complete || System.getenv("SVARCADE_ARCADE_ONLY") == "1") return
        bootTicks++
        ...
        if (!syntheticSpeciesReady) {
            if (bootTicks > 1200) {
                throw IllegalStateException("SVArcade visual smoke could not seed title-screen Cobblemon species")
            }
            return
        }
        ...
        if (!rendererReady) {
            ...
            System.out.println("[SVArcade Visual Smoke] Cobblemon model repository ready for embedded actors")
        }
        ...
        if (arena == null) {
            if (bootTicks > 1200) {
                throw IllegalStateException("SVArcade visual smoke timed out waiting for arena definition: $arenaId")
            }
            return
        }
        ...
        if (active?.arenaId != arenaId) {
            ...
            System.out.println("[SVArcade Visual Smoke] opened $arenaId")
            return
        }
        ...
        if (!capturedCurrent && stableTicks >= 70 && active.fixtureReady && resolvedActors >= EXPECTED_ACTORS) {
            ...
            val fileName = "svarcade-tft-$arenaId.png"
            Screenshot.grab(client.gameDirectory, fileName, client.mainRenderTarget) { message ->
                System.out.println("[SVArcade Visual Smoke] captured $fileName with $resolvedActors actors :: ${message.string}")
            }
        }

        if (!capturedCurrent && stableTicks > 600) {
            throw IllegalStateException(
                "SVArcade visual smoke timed out waiting for Cobblemon actors in $arenaId: " +
    ...
    private fun tickUiScenario(client: Minecraft) {
        ...
        if (active?.scenario != scenario) {
            ...
            System.out.println("[SVArcade Visual Smoke] opened full TFT UI scenario " + scenario)
            return
        }
        ...
        if (!capturedCurrent && stableTicks >= captureAt && active.fixtureReady && resolvedActors >= requiredActors) {
            capturedCurrent = true
            val fileName = "svarcade-tft-ui-" + scenario + ".png"
            Screenshot.grab(client.gameDirectory, fileName, client.mainRenderTarget) { message ->
                System.out.println("[SVArcade Visual Smoke] captured " + fileName + " with " + resolvedActors + " actors :: " + message.string)
            }
        }
        ...
        if (!capturedCurrent && stableTicks > 600) {
            throw IllegalStateException(
                "SVArcade full TFT UI smoke timed out in " + scenario + ": " +
                    resolvedActors + "/" + EXPECTED_ACTORS + " resolved; fixtureReady=" + active.fixtureReady
            )
        }
    ...
    private fun tickGameplayScenario(client: Minecraft) {
        ...
        if (active?.scenario != scenario) {
            ...
            System.out.println("[SVArcade Visual Smoke] opened gameplay scenario " + scenario)
            return
        }
        stableTicks++
        ...
        if (!capturedCurrent && stableTicks >= 70 && active.fixtureReady && resolvedActors >= requiredActors) {
            capturedCurrent = true
            val fileName = "svarcade-gameplay-" + scenario + ".png"
            Screenshot.grab(client.gameDirectory, fileName, client.mainRenderTarget) { message ->
                System.out.println("[SVArcade Visual Smoke] captured " + fileName + " with " + resolvedActors + " actors :: " + message.string)
            }
        }
        if (!capturedCurrent && stableTicks > 600) {
            throw IllegalStateException(
                "SVArcade gameplay smoke timed out in " + scenario + ": " +
    ...
    private fun tickResultScenario(client:Minecraft) {
        ...
        if(active?.scenario!=scenario) {
            ...
            System.out.println("[SVArcade Visual Smoke] opened result scenario "+scenario)
            return
        }
        stableTicks++
        if(!capturedCurrent && stableTicks>=60 && active.fixtureReady) {
            capturedCurrent=true
            val fileName="svarcade-result-"+scenario+".png"
            Screenshot.grab(client.gameDirectory,fileName,client.mainRenderTarget){message->
                System.out.println("[SVArcade Visual Smoke] captured "+fileName+" :: "+message.string)
            }
        }
        if(!capturedCurrent && stableTicks>500) throw IllegalStateException("SVArcade result smoke timed out in "+scenario)
    ...
    private fun tickSkinShowcase(client:Minecraft) {
        ...
        if(active==null) {
            ...
            System.out.println("[SVArcade Visual Smoke] opened skin showcase")
            return
        }
        stableTicks++
        ...
        if(stableTicks>=60 && active.fixtureReady && resolved) {
            ...
            Screenshot.grab(client.gameDirectory,"svarcade-skin-showcase.png",client.mainRenderTarget){message->
                System.out.println("[SVArcade Visual Smoke] captured svarcade-skin-showcase.png :: "+message.string)
            }
            stableTicks=0
        } else if(stableTicks>600) {
            throw IllegalStateException("SVArcade skin showcase smoke timed out: resolved="+resolved+" fixtureReady="+active.fixtureReady)
        }
    }
    ...
    private fun tickStoreScenario(client:Minecraft) {
        if(storeScenarioIndex>=storeScenarios.size) {
            ...
            if(finalWaitTicks>=40) {
                ...
                System.out.println("[SVArcade Visual Smoke] completed "+total+" captures; stopping client")
                enabled=false
                client.stop()
            }
            return
        ...
        if(active?.scenario!=scenario) {
            ...
            System.out.println("[SVArcade Visual Smoke] opened store scenario "+scenario)
            return
        }
        stableTicks++
        val needsPortrait=scenario.startsWith("tactician")
        val tacticianId=if(needsPortrait) "svarcade:greninja" else "svarcade:pikachu"
        ...
        if(!capturedCurrent && stableTicks>=55 && active.fixtureReady && resolved) {
            capturedCurrent=true
            val fileName="svarcade-store-"+scenario+".png"
            Screenshot.grab(client.gameDirectory,fileName,client.mainRenderTarget){message->
                System.out.println("[SVArcade Visual Smoke] captured "+fileName+" :: "+message.string)
            }
        }
        if(!capturedCurrent && stableTicks>600) {
            throw IllegalStateException("SVArcade store smoke timed out in "+scenario+": actor="+expectedActor+" resolved="+resolved)
        }
     ...
     * This path is unreachable unless SVARCADE_VISUAL_SMOKE=1.
     */
    private fun prepareSyntheticSpecies() {
        ...
        runCatching {
            ...
            if (added > 0) {
                ...
                System.out.println("[SVArcade Visual Smoke] seeded $added title-screen Pokemon species")
            }
    ...
    private class TftUiVisualSmokeScreen(val scenario: String) : Screen(Component.literal("SVArcade TFT UI Visual Smoke")) {
        ...
        override fun render(gui: GuiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
            ...
            TftGameRenderer.render(
                ...
                font = font,
                area = area,
                density = io.github.aristheg201.svarcade.ui.NativeLayout.resolve(width,height).density,
                view = view,
                ui = ui,
                mouseX = mouseX,
                mouseY = mouseY,
        ...
        companion object {
            private val authoredCatalogs by lazy {
                io.github.aristheg201.svarcade.native.game.TftSession(
                    listOf(NativeSeat("catalog-a","A"),NativeSeat("catalog-b","B")),seed=19L,
                    definition=io.github.aristheg201.svarcade.native.game.tft.TftSetRegistry.active()
            ...
            fun fixtureView(scenario: String): JsonObject {
                ...
                val fields = JsonObject().apply {
                    ...
                    addProperty("tacticianId", if (pve) "svarcade:shiny_mewtwo" else "svarcade:pikachu")
    ...
    private class NativeGameVisualSmokeScreen(val scenario:String):Screen(Component.literal("SVArcade Gameplay Visual Smoke")) {
    ...
    private class ResultVisualSmokeScreen(val scenario:String):Screen(Component.literal("SVArcade Result Visual Smoke")) {
    ...
    private class SkinShowcaseVisualSmokeScreen:Screen(Component.literal("SVArcade Skin Showcase Visual Smoke")) {
        private var rendered=false
        val fixtureReady get()=rendered
        override fun isPauseScreen():Boolean=false
        override fun render(gui:GuiGraphics,mouseX:Int,mouseY:Int,partialTick:Float) {
    ...
    private class StoreVisualSmokeScreen(val scenario:String):Screen(Component.literal("SVArcade Store Visual Smoke")) {
        private val ui=CosmeticStoreUi().also { state ->
            state.kind=if(scenario.startsWith("tactician"))"TACTICIAN" else "ARENA"
            state.selected=if(state.kind=="TACTICIAN")"svarcade:greninja" else "grand_line"
        }
        ...
        companion object {
            private fun fixture(scenario:String)=JsonObject().apply {
                ...
                addProperty("tactician",if(tactician&&equipped)"svarcade:greninja" else "svarcade:pikachu")
                ...
                add("offers",JsonArray().apply {
                    ...
                    add(storeOffer("TACTICIAN","svarcade:pikachu","0","TestGems",true,tactician&&!equipped,
                        name="Pikachu",species="cobblemon:pikachu",scale=.70))
                    add(storeOffer("TACTICIAN","svarcade:greninja","350","TestGems",tactician&&owned,tactician&&equipped,
                        name="Greninja",species="cobblemon:greninja",scale=.72))
                    add(storeOffer("TACTICIAN","svarcade:mewjo","650","TestGems",false,false,
    ...
    private class ArenaVisualSmokeScreen(val arenaId: String) : Screen(Component.literal("SVArcade Visual Smoke")) {
        ...
        override fun render(gui: GuiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
            ...
            val entities = cachedEntities ?: smokeEntities(arena).takeIf { it.size == EXPECTED_ACTORS }?.also {
                cachedEntities = it
                System.out.println("[SVArcade Visual Smoke] fixture ready $arenaId with ${it.size} Pokemon")
            }.orEmpty()
            ...
            gui.drawString(
                font,
                "SVArcade TFT · $arenaId · actors ${PokemonModelRenderer.sceneSizingDiagnostics().count { it.instanceId.startsWith("visual:$arenaId:") }}/$EXPECTED_ACTORS",
                11,
                11,
                0xFFFFFFFF.toInt(),
                true
