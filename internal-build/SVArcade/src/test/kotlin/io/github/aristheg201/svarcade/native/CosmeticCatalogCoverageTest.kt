...
class CosmeticCatalogCoverageTest {
    ...
    fun `bundled cosmetic catalog covers every authored TFT arena and tactician`() {
        val set = checkNotNull(javaClass.getResourceAsStream("/data/svarcade/tft/sets/kanto_rising/set.json"))
            .bufferedReader().use { JsonParser.parseReader(it).asJsonObject }
        val catalog = checkNotNull(javaClass.getResourceAsStream("/data/svarcade/cosmetic_store.json"))
