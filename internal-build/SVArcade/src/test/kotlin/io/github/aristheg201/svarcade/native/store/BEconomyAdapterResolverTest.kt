...
class BEconomyAdapterResolverTest {
    ...
    @Test
    fun `default adapter resolves live BlanketEconomy package and both SVArcade currencies`() {
        ...
        assertEquals(BigDecimal("2000"), adapter.balance(player, "BeastCoin"))
        assertEquals(BigDecimal("1500"), adapter.balance(player, "HunterCoin"))
    }
    ...
    fun `default bridge performs BeastCoin and HunterCoin mutations through provider API`() {
        ...
        assertTrue(adapter.debit(player, BigDecimal("250"), "BeastCoin"))
        adapter.credit(player, BigDecimal("25.50"), "HunterCoin")

        assertEquals(BigDecimal("1750"), adapter.balance(player, "BeastCoin"))
        assertEquals(BigDecimal("1525.50"), adapter.balance(player, "HunterCoin"))
    }
    ...
    fun `custom recovery receipt is read from actual provider transaction shape`() {
        ...
        adapter.recordReceipt(player, identity, amount, "BeastCoin")

        assertTrue(adapter.receipt(player, identity, amount, "BeastCoin"))
        assertFalse(adapter.receipt(player, identity + ":other", amount, "BeastCoin"))
    }
    ...
    fun `BEconomy currency aliases resolve to canonical server config ids`() {
        ...
        val status = adapter.status()
        assertTrue(status.ready)
        assertEquals(BigDecimal("900"), adapter.balance(player, "BeastCoin"))
        assertEquals(BigDecimal("450"), adapter.balance(player, "HunterCoin"))

        assertTrue(adapter.debit(player, BigDecimal("100"), "BeastCoin"))
        adapter.credit(player, BigDecimal("25"), "HunterCoin")

        assertEquals(BigDecimal("800"), adapter.balance(player, "BeastCoin"))
        assertEquals(BigDecimal("475"), adapter.balance(player, "HunterCoin"))
        assertTrue(status.detail.contains("beastcoin"))
        assertTrue(status.detail.contains("hunter_coin"))
    }

    @Test
    fun `required currency health reports missing currency instead of silently falling back`() {
        BlanketEconomy.api.balances.remove("HunterCoin")
        val status = BEconomyAdapter().status()

        assertTrue(status.available)
        assertTrue(status.ready)
        assertFalse(BEconomyAdapter().currencyExists("HunterCoin"))
    }
}
