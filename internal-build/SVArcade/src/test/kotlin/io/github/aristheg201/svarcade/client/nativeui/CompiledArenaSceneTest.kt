...
import io.github.aristheg201.svarcade.ui.SceneMeshNode
import io.github.aristheg201.svarcade.ui.SceneBlockModelNode
...
import io.github.aristheg201.svarcade.ui.PerspectiveBoardTransform

class CompiledArenaSceneTest {
    @Test fun everyBundledArenaPassesTransactionalReloadValidation() {
        java.nio.file.Files.list(java.nio.file.Path.of("src/main/resources/assets/svarcade/arenas")).use { paths ->
    ...
    @Test fun authoredArenasFrameTheBoardAndEveryBenchSlot() {
        for(id in listOf("gotham_rooftops","sector_2814","kanto_stadium","monster_island","grand_line","celestial_vault","dragon_shrine","distortion_rift","ultra_lab","ancient_ruins","temporal_observatory","abyssal_sanctum","crimson_caldera","arkham_asylum","wayne_manor","infinite_void","sukuna_domain")) {
            val arena=checkNotNull(javaClass.getResourceAsStream("/assets/svarcade/arenas/$id.json")).bufferedReader().use { MinecraftArenaRegistry.parse(JsonParser.parseReader(it).asJsonObject) }
            for((w,h) in listOf(632 to 270,952 to 414,340 to 250)) {
                val area=UiRect(102,40,w,h)
                val preset=arena.camera(ArenaCameraRole.PREPARATION,io.github.aristheg201.svarcade.ui.SceneCameras.TFT)
                val framed=io.github.aristheg201.svarcade.ui.SceneCameraFraming.board(preset,area,SceneVec3(0.0,0.0,0.0),7,8,arena.benchAnchors.map { SceneVec3(it.x.toDouble(),it.y.toDouble(),it.z.toDouble()) },cellSize=SceneVec3(arena.cellSize.x.toDouble(),arena.cellSize.y.toDouble(),1.0))
                ...
                val obstacles=arena.geometry.map { it.id to io.github.aristheg201.svarcade.ui.SceneBounds.enclosing(it.corners()) }.filter { it.second.max.z>.15 }
    ...
    @Test fun texturedBattlefieldsDoNotRetainTheLegacyFlatFloor() {
        ...
        for(id in listOf("gotham_rooftops","sector_2814","kanto_stadium","monster_island","grand_line","celestial_vault","dragon_shrine","distortion_rift","ultra_lab","ancient_ruins","temporal_observatory","abyssal_sanctum","crimson_caldera","arkham_asylum","wayne_manor","infinite_void","sukuna_domain")) {
            val arena=checkNotNull(javaClass.getResourceAsStream("/assets/svarcade/arenas/$id.json")).bufferedReader().use {
    ...
    @Test fun premiumArenaPropsUseAuthoredModelKindsWithoutRegistryBootstrap() {
        val layout=PokemonSceneLayout(UiRect(0,0,800,600),7,8,400f,30f,28,14)
        fun nodes(id:String)=checkNotNull(javaClass.getResourceAsStream("/assets/svarcade/arenas/$id.json")).bufferedReader().use {
    ...
    @Test fun premiumHunterCoinArenasHaveMateriallyDistinctSceneIdentity() {
        fun load(id:String)=checkNotNull(javaClass.getResourceAsStream("/assets/svarcade/arenas/$id.json"))
    ...
    @Test fun gothamAndSectorCompileFromMateriallyDifferentAuthoredScenes() {
        fun load(id: String): MinecraftArenaDefinition {
            val stream = checkNotNull(javaClass.getResourceAsStream("/assets/svarcade/arenas/$id.json"))
    ...
    @Test fun everyPlayableArenaHasDistinctGeometryBeyondMaterialRecolors() {
        val set = io.github.aristheg201.svarcade.native.game.tft.TftSetRegistry.bundled("kanto_rising")
