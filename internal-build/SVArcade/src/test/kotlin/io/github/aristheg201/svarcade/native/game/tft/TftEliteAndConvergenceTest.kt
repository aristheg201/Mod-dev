...
import io.github.aristheg201.svarcade.engine.Stat
...
class TftEliteAndConvergenceTest {
    ...
    @Test fun eliteAmplifierDoublesActualPhysicalMagicAndTrueDamageWithoutBoostingOtherFranchises() {
        ...
        val world = io.github.aristheg201.svarcade.engine.BattleRuntime(
            io.github.aristheg201.svarcade.engine.BattleBoard(7,8,true,emptySet()),
            io.github.aristheg201.svarcade.engine.BattleRuntime.Limits(12,4096,1024,2048,128), 1L,
            { _,_,_,_,_ -> error("No summons") }, emptyMap())
        for (kind in io.github.aristheg201.svarcade.engine.DamagePipeline.Type.entries) {
            fun damage(id: String): Double {
                val source = io.github.aristheg201.svarcade.engine.BattleUnit.restore(snapshots.getValue(id))
                val target = io.github.aristheg201.svarcade.engine.BattleUnit.restore(snapshots.getValue("enemy"))
                target.shield = 0.0
                return io.github.aristheg201.svarcade.engine.DamagePipeline.damage(world,source,target,10.0,kind,false,0.0,0)
            }
