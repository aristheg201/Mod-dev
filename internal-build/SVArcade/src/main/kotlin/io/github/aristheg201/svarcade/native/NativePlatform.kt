...
object NativePlatform {
    ...
    fun open(player: ServerPlayer, requested: String): Boolean {
        ...
        if (!NativeProfileStore.isLoaded(player.uuid)) {
            ...
            player.sendSystemMessage(net.minecraft.network.chat.Component.translatable("message.svarcade.profile_loading"))
            return false
        }
        pendingOpen.remove(player.uuid)
    ...
    fun handleIntent(player: ServerPlayer, module: String, action: String, data: JsonObject): String {
        if (!NativeProfileStore.isLoaded(player.uuid)) return "gui.svarcade.profile.loading"
        ...
        return when (module) {
            ...
            "wallet", "dashboard" -> when (action) {
                ...
                else -> "gui.svarcade.error.no_action"
            }
            else -> "gui.svarcade.error.invalid_module"
        }
    }
    ...
    private fun handleGacha(player: ServerPlayer, action: String, data: JsonObject) = when (action) {
        ...
        else -> "gui.svarcade.error.invalid_action"
    }
    ...
    private fun handleArcade(player: ServerPlayer, action: String, data: JsonObject) = when (action) {
        ...
        "resume" -> { val s = gameState(player); if (s.get("empty")?.asBoolean == false) NativePlatformNetwork.sendOpen(player, "game", s) else NativePlatformNetwork.sendState(player, "arcade", NativeArcadeService.lobbyState(player), "gui.svarcade.arcade.no_active"); "" }
    ...
    private fun handleGame(player: ServerPlayer, action: String, data: JsonObject): String {
        ...
        if (action != "act") return "gui.svarcade.error.invalid_action"
    ...
    private fun handleCompanions(player: ServerPlayer, action: String, data: JsonObject): String {
        val msg = when (action) {
            "select" -> { val entity = data.string("entity"); if (VanillaCompanionService.select(player, entity)) "gui.svarcade.companion.selected" else "gui.svarcade.companion.invalid" }
            "arena_start" -> {
                val selected = VanillaCompanionService.selectedFor(player.uuid)
                if (selected.isNullOrBlank()) "gui.svarcade.arena.select_first"
                else { CompanionArena.start(player.uuid, selected); "gui.svarcade.arena.started" }
            }
            "arena_act" -> { CompanionArena.act(player.uuid, data.string("move")); "gui.svarcade.arena.updated" }
            else -> "gui.svarcade.error.invalid_action"
        }
