...
class ChunkAssembler(
    ...
    fun accept(transferId: Long, index: Int, total: Int, chunk: String): String? {
        require(total in 1..MAX_CHUNKS) { "Invalid SVArcade chunk count: $total" }
        require(index in 0 until total) { "Invalid SVArcade chunk index: $index/$total" }
        require(chunk.length <= Compression.CHUNK_CHARS) { "SVArcade chunk exceeds maximum size" }

        cleanupExpired()

        var transfer = transfers[transferId]
        if (transfer == null) {
            require(transfers.size < maxActiveTransfers) { "Too many concurrent SVArcade transfers" }
            transfer = Transfer(System.currentTimeMillis(), total, arrayOfNulls(total))
            transfers[transferId] = transfer
        }
        require(transfer.total == total) { "SVArcade transfer metadata changed mid-stream" }

        val existing = transfer.chunks[index]
        if (existing != null) {
            require(existing == chunk) { "Conflicting duplicate SVArcade chunk" }
            return null
        }

        val nextChars = transfer.receivedChars + chunk.length
        require(nextChars <= Compression.MAX_TRANSFER_CHARS) { "SVArcade transfer exceeds maximum compressed payload" }
