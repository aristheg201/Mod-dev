...
object PokemonRuntimeInfoService {
    ...
    private fun ensureIndex(server:MinecraftServer){if(!building.compareAndSet(false,true))return;val pool=executor?:run{building.set(false);return};val details=runCatching{CobblemonSpawnPools.WORLD_SPAWN_POOL.details.toList()}.getOrElse{SVArcade.LOGGER.warn("Unable to snapshot Cobblemon spawn pool",it);emptyList()};try{pool.execute{val built=runCatching{buildIndex(details)};server.execute{building.set(false);built.onSuccess{idx->spawnIndex=idx;val requests=queued.values.toList();queued.clear();requests.forEach{r->ResourceLocation.tryParse(r.species)?.let(PokemonSpecies::getByIdentifier)?.let{s->dispatch(server,r,idx,s)}}}.onFailure{SVArcade.LOGGER.warn("Unable to build SVArcade spawn index",it)}}}}catch(_:RejectedExecutionException){building.set(false)}}
