...
object EconomyConfig {
    ...
    fun start(path: Path) {
        if (!Files.exists(path)) AtomicFiles.writeUtf8(path, requireNotNull(javaClass.getResourceAsStream("/data/svarcade/economy.json")).bufferedReader().use { it.readText() })
