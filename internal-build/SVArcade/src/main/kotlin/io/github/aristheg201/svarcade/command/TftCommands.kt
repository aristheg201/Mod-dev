...
import io.github.aristheg201.svarcade.native.game.tft.PokemonPresentationDiagnostics
...
object TftCommands {
    fun register() = CommandRegistrationCallback.EVENT.register { dispatcher, _, _ ->
        val admin = Commands.literal("tft").requires { SVArcadePermissions.check(it, SVArcadePermissions.ADMIN_DEBUG, 3) }
            .then(Commands.literal("config")
                ...
                .then(Commands.literal("preview-aspect").then(speciesArgument().then(Commands.argument("aspects", StringArgumentType.greedyString()).suggests { ctx, b ->
                    ...
                    if (unknown.isNotEmpty()) { ctx.source.sendFailure(Component.literal("Unresolved aspects for $species: $unknown")); 0 }
                        ...
                        val identity=io.github.aristheg201.svarcade.native.game.tft.PokemonPresentationIdentity(species=species,aspects=requested)
                ...
                .then(Commands.literal("generate").then(Commands.literal("team").then(Commands.argument("id", StringArgumentType.word()).executes { ctx ->
                    val id = StringArgumentType.getString(ctx, "id"); ctx.source.sendSuccess({ Component.literal("{\"id\":\"svarcade:$id\",\"name\":\"$id\",\"members\":[],\"bench\":[],\"aiProfile\":\"normal\"}") }, false); 1
                })))
        ...
        dispatcher.register(Commands.literal("svarcade").then(admin))
    }
