...
object NativeSkinService {
    ...
    fun purchase(player:ServerPlayer,skinId:String):Result{val skin=NativeSkinCatalog.get(skinId)?:return Result(false,"gui.svarcade.skin.not_found");val ok=SkiesSkinsBridge.openShop(player,skin.source);return Result(ok,if(ok)"gui.svarcade.skin.shop_opened" else "gui.svarcade.skin.backend_unavailable")}
    fun equip(player:ServerPlayer,skinId:String,slot:Int)=SkiesSkinsBridge.apply(player,skinId,slot).let{Result(it.ok,if(it.ok)"gui.svarcade.skin.equipped" else "gui.svarcade.skin.operation_failed")}
    fun unequip(player:ServerPlayer,slot:Int)=SkiesSkinsBridge.remove(player,slot).let{Result(it.ok,if(it.ok)"gui.svarcade.skin.removed" else "gui.svarcade.skin.operation_failed")}
    fun openInventory(player:ServerPlayer):Result{val ok=SkiesSkinsBridge.openInventory(player);return Result(ok,if(ok)"gui.svarcade.skin.inventory_opened" else "gui.svarcade.skin.backend_unavailable")}
    ...
    internal fun bonusPokemonUuidForRequest(requestId:String):UUID =
        UUID.nameUUIDFromBytes(("svarcade:gacha-bonus:"+requestId).toByteArray(StandardCharsets.UTF_8))
