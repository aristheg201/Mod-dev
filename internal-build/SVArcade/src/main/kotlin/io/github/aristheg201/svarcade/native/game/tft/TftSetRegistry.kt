...
object TftSetRegistry {
    ...
    private val logger = LoggerFactory.getLogger("SVArcade/TFT")
    @Volatile private var current: TftSetDefinition? = null
    @Volatile private var overrideFile: Path? = null
    private val reloadWorker = Executors.newSingleThreadExecutor { task -> Thread(task, "SVArcade-TFT-Reload").apply { isDaemon = true } }
    ...
    internal fun migrateDefinition(set: TftSetDefinition): TftSetDefinition {
        ...
        val manifest = open("/data/svarcade/tft/sets/kanto_rising/set.json").use { reader ->
    ...
    internal fun bundled(id: String): TftSetDefinition {
        ...
        val root = "/data/svarcade/tft/sets/$id"
