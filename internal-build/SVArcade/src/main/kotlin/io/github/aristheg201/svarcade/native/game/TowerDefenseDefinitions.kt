...
object TowerDefenseDefinitions {
    ...
    val bundled:TowerDefenseDefinition by lazy {
        val stream=TowerDefenseDefinitions::class.java.getResourceAsStream("/data/svarcade/tower_defense/default.json") ?: error("Missing bundled TD definition")
        validate(stream.reader().use { gson.fromJson(it,TowerDefenseDefinition::class.java) },"data/svarcade/tower_defense/default.json")
    }
    private val published=AtomicReference<TowerDefenseDefinition>()
    private val loader=Executors.newSingleThreadExecutor{r->Thread(r,"SVArcade-TD-Definition").apply{isDaemon=true}}
