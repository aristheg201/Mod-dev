...
import io.github.aristheg201.svarcade.native.game.NativeBotAction
import io.github.aristheg201.svarcade.native.game.NativeGameResult
import io.github.aristheg201.svarcade.native.game.NativeGameSession
    ...
    internal class Actor(
        ...
        private fun drain() {
            try {
                ...
                SVArcade.LOGGER.error("Native session actor crashed: {} / {}", gameId, sessionId, error)
            } finally {
                scheduled.set(false)
        ...
        private fun processTick(command: Command.Tick) {
            val changed = runCatching { session.tick(command.nowMillis) }.getOrElse {
                SVArcade.LOGGER.warn("Native game tick failed for {} / {}", gameId, sessionId, it); false
            }
            ...
            if (changed || finishChanged) {
                val update = Update(sessionId, null, false, null, true, changed, if (finished) "gui.svarcade.game.finished" else "", finished, winnerSeatId)
        ...
        private fun publicMessage(result: NativeGameResult): String = when {
            ...
            result.accepted -> "gui.svarcade.game.action_applied"
            else -> "gui.svarcade.game.action_rejected"
        }
        ...
        private fun captureState(nowMillis: Long): JsonObject =
            runCatching { session.snapshotState(nowMillis) }
                .onFailure { error -> SVArcade.LOGGER.warn("Native game snapshot failed for {} / {}", gameId, sessionId, error) }
    ...
    fun start() {
        ...
        synchronized(this) {
            ...
            executor = ThreadPoolExecutor(
                ...
                { task -> Thread(task, "SVArcade-Game-${threadCounter.incrementAndGet()}").apply { isDaemon = true; priority = Thread.NORM_PRIORITY - 1 } },
