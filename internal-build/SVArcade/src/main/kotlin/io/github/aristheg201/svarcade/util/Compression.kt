...
object Compression {
    ...
    fun decodeUtf8(encoded: String): String {
        require(encoded.length <= MAX_TRANSFER_CHARS) { "SVArcade transfer exceeds maximum compressed payload" }
        ...
        return GZIPInputStream(ByteArrayInputStream(bytes)).use { gzip ->
            ...
            while (true) {
                ...
                if (read < 0) break
                total += read
                require(total <= MAX_DECOMPRESSED_BYTES) { "SVArcade payload exceeds maximum decompressed size" }
