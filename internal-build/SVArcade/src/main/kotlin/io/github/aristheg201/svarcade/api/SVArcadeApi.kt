package io.github.aristheg201.svarcade.api

import io.github.aristheg201.svarcade.content.HubActionSpec
...
object SVArcadeApi {
    ...
    fun registerIntegration(descriptor: HubIntegrationDescriptor) {
        require(idPattern.matches(descriptor.id)) { "Invalid SVArcade integration id '${descriptor.id}'" }
        val previous = integrations.putIfAbsent(descriptor.id, descriptor)
        require(previous == null || previous == descriptor) { "SVArcade integration '${descriptor.id}' is already registered" }
    }

    @JvmStatic
    fun registerServerActionType(type: String, handler: ServerActionHandler) {
        require(idPattern.matches(type)) { "Invalid SVArcade action type '$type'" }
        require(type !in CORE_ACTION_TYPES) { "'$type' is a core SVArcade action type and cannot be replaced" }
        val previous = actionHandlers.putIfAbsent(type, handler)
        require(previous == null || previous === handler) { "SVArcade action type '$type' is already registered" }
    }
