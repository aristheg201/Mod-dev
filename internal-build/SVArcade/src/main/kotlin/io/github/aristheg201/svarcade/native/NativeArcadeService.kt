...
object NativeArcadeService {
    ...
    fun start(player: ServerPlayer, gameId: String, requestedMode: String, timeControl: String = "10+5"): Result {
        if (!NativeArcadeSessionStore.isLoadComplete()) {
            return Result(false, "gui.svarcade.arcade.recovering")
        }
        restoreLoadedSessions(player.server, System.currentTimeMillis())
        val def = games.firstOrNull { it.id == gameId } ?: return Result(false, "gui.svarcade.arcade.invalid_game")
        if (sessions.size >= MAX_SESSIONS) return Result(false, "gui.svarcade.arcade.server_busy")
        val mode = if (requestedMode == "bot") "bot_normal" else requestedMode
        if (mode !in def.modes) return Result(false, "gui.svarcade.arcade.invalid_mode")
        ...
        when (NativeArcadeLifecyclePolicy.resolve(
            ...
            ActiveSessionResolution.RESUME -> {
                disconnectedUntil.remove(player.uuid)
                return Result(true, "gui.svarcade.arcade.resumed", setOf(player.uuid))
            }
        ...
        if (mode == "pvp" || mode == "ranked") {
            ...
            if (gameId == "tft" && mode == "pvp") {
                ...
                if (decision != TftLifecyclePolicy.CollectionDecision.START) {
                    ...
                    return Result(true, "gui.svarcade.arcade.queued", ready.map { it.uuid }.toSet())
                }
                tftCollectionDeadline.remove(gameId)
                val handle = startTftPvp(player.server, ready)
                return Result(true, "gui.svarcade.arcade.matched", realPlayers(handle))
            }
            while (q.isNotEmpty()) {
            ...
            return Result(true, "gui.svarcade.arcade.queued", setOf(player.uuid))
        }
        ...
        val handle = register(player.server, create(gameId, seats, timeControl), mode)
        return Result(true, "gui.svarcade.arcade.started", realPlayers(handle))
    }

    fun cancelQueue(player: ServerPlayer): Result {
        var removed = false
        queues.values.forEach { removed = it.remove(player.uuid) || removed }
        return Result(removed, if (removed) "gui.svarcade.arcade.queue_left" else "gui.svarcade.arcade.not_queued", setOf(player.uuid))
    }

    fun act(player: ServerPlayer, action: String, args: Map<String, String>): Result {
        val sid = active[player.uuid] ?: return Result(false, "gui.svarcade.arcade.no_active")
        val handle = sessions[sid] ?: return Result(false, "gui.svarcade.arcade.session_expired")
        ...
        return if (queued) Result(true, "gui.svarcade.arcade.processing") else Result(false, "gui.svarcade.arcade.worker_busy", setOf(player.uuid))
    }

    fun leave(player: ServerPlayer): Result {
        cancelQueue(player)
        disconnectedUntil.remove(player.uuid)
        val sid = active[player.uuid] ?: return Result(true, "gui.svarcade.arcade.left", setOf(player.uuid))
        ...
        return Result(true, "gui.svarcade.arcade.left", setOf(player.uuid))
    }

    fun rematch(player: ServerPlayer): Result {
        val sid=active[player.uuid] ?: return Result(false,"gui.svarcade.arcade.no_active")
        val handle=sessions[sid] ?: return Result(false,"gui.svarcade.arcade.no_active")
        if(!handle.finished) return Result(false,"gui.svarcade.result.rematch_unavailable")
        val mode=meta[sid]?.mode ?: return Result(false,"gui.svarcade.result.rematch_unavailable")
    ...
    fun tick(server: MinecraftServer, now: Long = System.currentTimeMillis()): Map<UUID, String> {
        ...
        disconnectedUntil.entries.toList().forEach { (id, deadline) ->
            ...
            if (TftLifecyclePolicy.disconnectExpiresToBot(handle.gameId)) {
                ...
                asyncMessages[id] = "gui.svarcade.arcade.bot_takeover"
            } else {
    ...
    private fun restoreLoadedSessions(server: MinecraftServer, now: Long) {
        ...
        records.forEach { record ->
            ...
            if (games.none { it.id == record.gameId }) {
                SVArcade.LOGGER.warn("Ignoring recovery for unknown native game {}", record.gameId)
                return@forEach
            }
            if (sessions.size >= MAX_SESSIONS) {
                SVArcade.LOGGER.error("Cannot restore native session {}; session capacity reached", record.sessionId)
                return@forEach
            }
            ...
            val realIds = record.seats.asSequence()
                ...
                .toSet()
            if (realIds.isEmpty()) {
                SVArcade.LOGGER.warn("Ignoring native recovery {} with no real players", record.sessionId)
                return@forEach
            }
            if (realIds.any(active::containsKey)) {
                SVArcade.LOGGER.warn("Dropping duplicate older native recovery {}", record.sessionId)
            ...
            val restored = runCatching {
                ...
                SVArcade.LOGGER.error("Unable to restore native session {}", record.sessionId, error)
            ...
            realIds.forEach { id ->
                if (server.playerList.getPlayer(id) == null) {
                    ...
                    disconnectedUntil.remove(id)
                    asyncMessages[id] = "gui.svarcade.arcade.restored"
                }
            }
            persistSession(handle, now, force = true)
    ...
    private fun launchExpiredTftCollection(server: MinecraftServer, now: Long) {
        ...
        when (TftLifecyclePolicy.decision(ready.size, deadline, now, tftMatchmaking)) {
            ...
            TftLifecyclePolicy.CollectionDecision.START -> {
                ...
                realPlayers(handle).forEach { asyncMessages[it] = "gui.svarcade.arcade.matched" }
            }
        }
    }
